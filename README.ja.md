...
HopCoderX はデスクトップアプリとしても利用できます。[releases page](https://github.com/TaimoorSiddiquiOfficial/hopcoderx/releases) から直接ダウンロードするか、[hopcoderx.dev/download](https://hopcoderx.dev/download) を利用してください。
...
[agents](https://hopcoderx.dev/docs/agents) の詳細はこちら。

### ドキュメント

HopCoderX の設定については [**ドキュメント**](https://hopcoderx.dev/docs) を参照してください。
...
- 100% オープンソース
- 特定のプロバイダーに依存しません。[HopCoderX Bdr](https://hopcoderx.dev/zen) で提供しているモデルを推奨しますが、HopCoderX は Claude、OpenAI、Google、またはローカルモデルでも利用できます。モデルが進化すると差は縮まり価格も下がるため、provider-agnostic であることが重要です。
- そのまま使える LSP サポート
