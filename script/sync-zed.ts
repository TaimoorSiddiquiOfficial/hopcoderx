...
const FORK_REPO = "TaimoorSiddiquiOfficial/zed-extensions"
const UPSTREAM_REPO = "zed-industries/extensions"
const EXTENSION_NAME = "HopCoderX"
...
async function main() {
  ...
  // Delete any existing branches for HopCoderX updates
  ...
  const prResult =
    await $`gh pr create --repo ${UPSTREAM_REPO} --base main --head ${FORK_REPO.split("/")[0]}:${branchName} --title "Update ${EXTENSION_NAME} to v${cleanVersion}" --body "Updating HopCoderX extension to v${cleanVersion}"`
