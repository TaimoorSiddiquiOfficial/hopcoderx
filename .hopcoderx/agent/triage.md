...
- `HopCoderX web` command
- The desktop app itself

**Only** add if it's specifically about the Desktop application or `HopCoderX web` view. **Do not** add for terminal, TUI, or general HopCoderX issues.
...
Use for core server issues in `packages/HopCoderX/`, excluding `packages/HopCoderX/src/cli/cmd/tui/`.
...
TUI (`packages/HopCoderX/src/cli/cmd/tui/...`):
...
Core (`packages/HopCoderX/...`, excluding TUI subtree):

- thdxr for sqlite/snapshot/memory bugs and larger architectural core features
- jlongster for HopCoderX server + API feature work (tool currently remaps jlongster -> thdxr until assignable)
