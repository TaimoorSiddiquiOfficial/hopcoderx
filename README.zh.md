...
brew install TaimoorSiddiquiOfficial/tap/hopcoderx # macOS 和 Linux（推荐，始终保持最新）
brew install HopCoderX              # macOS 和 Linux（官方 brew formula，更新频率较低）
sudo pacman -S HopCoderX            # Arch Linux (Stable)
paru -S HopCoderX-bin               # Arch Linux (Latest from AUR)
mise use -g HopCoderX               # 任意系统
nix run nixpkgs#HopCoderX           # 或用 github:TaimoorSiddiquiOfficial/hopcoderx 获取最新 dev 分支
```
...
1. `$HOPCODERX_INSTALL_DIR` - 自定义安装目录
2. `$XDG_BIN_DIR` - 符合 XDG 基础目录规范的路径
3. `$HOME/bin` - 如果存在或可创建的用户二进制目录
4. `$HOME/.HopCoderX/bin` - 默认备用路径
...
HopCoderX 内置两种 Agent，可用 `Tab` 键快速切换：
...
如果你在项目名中使用了 “HopCoderX”（如 “HopCoderX-dashboard” 或 “HopCoderX-mobile”），请在 README 里注明该项目不是 HopCoderX 团队官方开发，且不存在隶属关系。
...
- 100% 开源。
- 不绑定特定提供商。推荐使用 [HopCoderX Bdr](https://hopcoderx.dev/bdr) 的模型，但也可搭配 Claude、OpenAI、Google 甚至本地模型。模型迭代会缩小差异、降低成本，因此保持 provider-agnostic 很重要。
- 内置 LSP 支持。
...
**加入我们的社区** [Discord](https://discord.gg/HopCoderX) | [X.com](https://x.com/HopCoderX)
