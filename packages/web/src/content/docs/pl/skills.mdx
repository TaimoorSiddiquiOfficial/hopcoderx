...
Umiejętności agenta pozwalają HopCoderX odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
HopCoderX przeszukuje te lokalizacje:
...
W przypadku ścieżek lokalnych projektu HopCoderX przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.HopCoderX/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/HopCoderX/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.HopCoderX/skills/git-release/SKILL.md` w ten sposób:
...
HopCoderX wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `HopCoderX.json`:
...
**Dla agentów wbudowanych** (w `HopCoderX.json`):
