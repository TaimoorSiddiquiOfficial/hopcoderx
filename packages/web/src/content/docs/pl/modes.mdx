...
:::caution
Tryby są teraz konfigurowane za pomocą opcji `agent` w konfiguracji HopCoderX.
...
te lub skonfiguruj własne za pomocą konfiguracji HopCoderX.
...
- `edit` — Nie można zastosować naruszenia plików, z naruszeniem praw autorskich w `.HopCoderX/plans/*.md` w celu uszczegółowienia samego planu
...
Skonfiguruj tryb w pliku konfiguracyjnym `HopCoderX.json`:
...
- Globalnie: `~/.config/HopCoderX/modes/`
...
Jeśli nie ma wpływu na temperaturę, HopCoderX stosuje się narzędzia badawcze dla modelu (zwykle 0 dla największych modeli, 0,55 dla modeli Qwen).
...
zarówno globalna HopCoderX, jak i specjalna dla projektu.
...
Utwórz pliki trybów w `.HopCoderX/modes/` dla trybów zapisanych dla projektu lub `~/.config/HopCoderX/modes/` dla trybów globalnych:
