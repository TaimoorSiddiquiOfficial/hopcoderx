...
W HopCoderX obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
HopCoderX zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
HopCoderX ma dwóch wbudowanych subagentów: **General** i **Explore**. Przyjrzymy się im poniżej.

---

## Wbudowane

HopCoderX ma dwa wbudowane agenty główne i dwa wbudowane subagenty.
...
Skonfiguruj agenta w pliku `HopCoderX.json`:
...
- Globalnie: `~/.config/HopCoderX/agents/`
- W projekcie: `.HopCoderX/agents/`
...
Jeśli nie zostanie skonfigurowana, HopCoderX używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji HopCoderX, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji HopCoderX w `provider/model-id`. Na przykład, [HopCoderX Bdr](/docs/zen), użyj `HopCoderX/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `HopCoderX models`, aby zobaczyć dostępne modele.
:::
...
:::tip
Masz agenta, którym warto się podzielić? [Prześlij PR](https://github.com/TaimoorSiddiquiOfficial/hopcoderx).
:::
