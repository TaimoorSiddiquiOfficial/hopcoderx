...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. HopCoderX posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w HopCoderX.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `HOPCODERX_EXPERIMENTAL_LSP_TOOL=true` (lub `HOPCODERX_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy HopCoderX lub gdy zmienna środowiskowa `HOPCODERX_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu HopCoderX:
