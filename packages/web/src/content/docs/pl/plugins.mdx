---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć HopCoderX.
---

Wtyczki umożliwiające wyłączenie HopCoderX poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania HopCoderX.
...
- `.HopCoderX/plugins/` - Wtyczki na poziomie projektu
- `~/.config/HopCoderX/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/HopCoderX/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/HopCoderX/HopCoderX.json`)
2. Konfiguracja projektu (`HopCoderX.json`)
3. Globalny katalog wtyczek (`~/.config/HopCoderX/plugins/`)
4. Katalog wtyczek projektu (`.HopCoderX/plugins/`)
...
HopCoderX uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z HopCoderX do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do HopCoderX.
...
:::note
Jeśli korzystasz z aplikacji komputerowej HopCoderX, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj HopCoderX czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do HopCoderX:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod HopCoderX. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://hopcoderx.dev/docs/sdk).
