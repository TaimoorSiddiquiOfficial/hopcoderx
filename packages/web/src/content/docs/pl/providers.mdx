---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w HopCoderX.
---

import config from "../../../../config.mjs"
export const console = config.console

HopCoderX używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji HopCoderX.
...
w `~/.local/share/HopCoderX/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim HopCoderX
config.
...
HopCoderX Bdr to lista modeli dostarczonych przez zespół HopCoderX, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z HopCoderX. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od HopCoderX Bdr.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz HopCoderX i przejdź do [hopcoderx.dev/auth](https://hopcoderx.dev/auth).
...
Działa jak każdy inny dostawca w HopCoderX i jest całkowicie opcjonalny w użyciu.
...
Aby używać Amazon Bedrock z HopCoderX:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania HopCoderX:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `HopCoderX.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w HopCoderX nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod HopCoderX działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji HopCoderX.
...
HopCoderX używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować HopCoderX
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`HopCoderX.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [HopCoderX-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth).

##### Konfiguracja

Customize through `HopCoderX.json`:
...
Aby korzystać z subskrypcji GitHub Copilot z kodem HopCoderX:
...
Aby używać Google Vertex AI z HopCoderX:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Set them while running HopCoderX.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą HopCoderX, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [HopCoderX-helicone-session](https://github.com/H2Shami/HopCoderX-helicone-session), aby automatycznie rejestrować każdą konwersację HopCoderX jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja HopCoderX będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować HopCoderX tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować HopCoderX tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować HopCoderX tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem HopCoderX. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/HopCoderX).
:::
...
Aby korzystać z Ollama Cloud z HopCoderX:
...
3. Skopiuj klucz API do użycia w HopCoderX.
...
6. **Ważne**: Przed użyciem modeli chmurowych w HopCoderX musisz pobrać informacje o modelu lokalnie:
...
HopCoderX Bdr to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół HopCoderX. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>HopCoderX Bdr</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **HopCoderX Bdr**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API HopCoderX.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji HopCoderX.
...
5. Możesz je także dostosować za pomocą konfiguracji HopCoderX. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z HopCoderX:
...
Możesz także dostosować modele za pomocą konfiguracji HopCoderX. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z HopCoderX. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `HopCoderX.json` w katalogu projektu:
...
Pola `limit` pozwalają HopCoderX zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `HopCoderX auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację HopCoderX i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji HopCoderX.
