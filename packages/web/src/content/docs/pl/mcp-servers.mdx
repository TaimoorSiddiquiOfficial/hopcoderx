...
Możesz dodać dodatkowe informacje do HopCoderX za pomocą _Model Context Protocol_, w skrócie MCP. HopCoderX obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [HopCoderX Config](https://hopcoderx.dev/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/HopCoderX`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
HopCoderX automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, HopCoderX:
...
Jeśli serwer wymaga uwierzytelnienia, HopCoderX poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `HopCoderX mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji HopCoderX bezpieczne przechowa tokeny w `~/.local/share/HopCoderX/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w HopCoderX, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie HopCoderX, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć HopCoderX z kontem Sentry.
