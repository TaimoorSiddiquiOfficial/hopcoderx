...
Aby debugować problemy z HopCoderX, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\HopCoderX\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `HopCoderX --log-level DEBUG`.

---

## Przechowywanie danych

HopCoderX przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/HopCoderX/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\HopCoderX`
...
HopCoderX Desktop uruchamia w tle lokalny serwer HopCoderX (sidecar `HopCoderX-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `HopCoderX` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/HopCoderX/HopCoderX.jsonc` (lub `~/.config/HopCoderX/HopCoderX.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/HopCoderX/HopCoderX.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\HopCoderX\HopCoderX.jsonc`
...
HopCoderX może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/HopCoderX/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\HopCoderX\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby HopCoderX mógł ją odbudować.

1. Całkowicie zamknij HopCoderX Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/HopCoderX`
- **Linux**: usuń `~/.cache/HopCoderX` (lub uruchom `rm -rf ~/.cache/HopCoderX`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\HopCoderX`

3. Uruchom ponownie HopCoderX Desktop.

---

### Napraw problemy z połączeniem z serwerem

HopCoderX Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `HopCoderX.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `HOPCODERX_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `HOPCODERX_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows HopCoderX Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami HopCoderX.

---

### Brak powiadomień

HopCoderX Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla HopCoderX w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie HopCoderX Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji HopCoderX Desktop):

- `HopCoderX.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `HopCoderX.global.dat` i `HopCoderX.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
Jeśli masz problemy z HopCoderX:
...
3. Upewnij się, że masz najnowszą wersję z `HopCoderX upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `HopCoderX models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\HopCoderX`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. HopCoderX dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\HopCoderX`

2. Uruchom ponownie kod HopCoderX, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to HopCoderX do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
HopCoderX wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
