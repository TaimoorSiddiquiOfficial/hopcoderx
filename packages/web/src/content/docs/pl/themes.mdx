...
Dzięki HopCoderX możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie HopCoderX używa naszego własnego motywu `HopCoderX`.
...
HopCoderX ma kilka wbudowanych motywów.
...
- Want HopCoderX to match their terminal's appearance
...
HopCoderX obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/HopCoderX/themes/*.json` lub `$XDG_CONFIG_HOME/HopCoderX/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.HopCoderX/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.HopCoderX/themes/*.json`
