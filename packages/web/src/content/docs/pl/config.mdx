---
title: Konfiguracja
description: Ustawienia HopCoderX w pliku JSON.
---

Możesz dostosować HopCoderX za pomocą pliku konfiguracyjnego JSON.

---

## Format

HopCoderX obsługuje formaty **JSON** i **JSONC** (JSON z komentarzami).
...
Na przykład, jeśli konfiguracja globalna ustawi `theme: "hopcoderx"` i `autoupdate: true`, a konfiguracja projektu ustawi `model: "anthropic/claude-sonnet-4-5"`, ostateczna konfiguracja będzie zawierać wszystkie trzy ustawienia.
...
1. **Konfiguracja zdalna** (z `.well-known/HopCoderX`) – ustawienia wymuszane przez organizację
2. **Konfiguracja globalna** (`~/.config/HopCoderX/HopCoderX.json`) – preferencje użytkownika
3. **Konfiguracja niestandardowa** (zmienna `HOPCODERX_CONFIG`) — jawne nadpisanie
4. **Konfiguracja projektu** (`HopCoderX.json` w projekcie) - ustawienia specyficzne dla projektu
5. **Katalogi `.HopCoderX`** - agenci, polecenia, umiejętności
6. **Konfiguracja wbudowana** (zmienna `HOPCODERX_CONFIG_CONTENT`) — nadpisanie środowiska uruchomieniowego
...
:::note
W katalogach `.HopCoderX` i `~/.config/HopCoderX` używane są **nazwy w liczbie mnogiej** dla podkatalogów: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Nazwy w liczbie pojedynczej (np. `agent/`) są również obsługiwane dla kompatybilności wstecznej.
:::
...
Organizacje mogą publikować konfigurację pod adresem `.well-known/HopCoderX`. Jest ona pobierana automatycznie po uwierzytelnieniu u dostawcy, który ją obsługuje.
...
Umieść swoją globalną konfigurację HopCoderX w `~/.config/HopCoderX/HopCoderX.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak motywy, domyślny dostawca lub skróty klawiszowe.
...
Dodaj `HopCoderX.json` w katalogu głównym projektu. Konfiguracja projektu ma najwyższy priorytet wśród plików konfiguracyjnych — nadpisuje konfiguracje globalne i zdalne.

:::tip
Dodaj ten plik do kontroli wersji, aby udostępniać konfigurację zespołowi.
:::

Kiedy HopCoderX się uruchamia, szuka pliku konfiguracyjnego w katalogu głównym repozytorium Git.
...
Możesz załadować niestandardowy plik konfiguracyjny, ustawiając zmienną środowiskową `HOPCODERX_CONFIG`.
...
Możesz ustawić niestandardowy katalog konfiguracyjny za pomocą zmiennej środowiskowej `HOPCODERX_CONFIG_DIR`. Katalog ten będzie przeszukiwany pod kątem agentów, poleceń, trybów i wtyczek, tak jak standardowy katalog `.HopCoderX` i powinien zachować tę samą strukturę.
...
Katalog niestandardowy jest ładowany po globalnej konfiguracji i katalogach `.HopCoderX`, więc **może być łączony** z ich zawartością.

---

## Schemat

Plik konfiguracyjny ma schemat JSON dostępny pod adresem [**`hopcoderx.dev/config.json`**](https://hopcoderx.dev/config.json).
...
Możesz skonfigurować serwer dla `HopCoderX serve` i `HopCoderX web` za pomocą opcji `server`.
...
- `mdns` - Włącz wykrywanie usług mDNS. Dzięki temu inne urządzenia w sieci mogą łatwo znaleźć Twój serwer HopCoderX.
- `mdnsDomain` - Niestandardowa nazwa domeny dla usług mDNS. Domyślnie `HopCoderX.local`. Przydatne do uruchamiania wielu instancji w tej samej sieci.
...
Opcja `small_model` konfiguruje oddzielny model do lżejszych zadań, takich jak generowanie tytułów. Domyślnie HopCoderX wybiera tańszy model od tego samego dostawcy, jeśli jest dostępny, w przeciwnym razie powraca do modelu głównego.
...
Skonfiguruj motyw interfejsu HopCoderX za pomocą opcji `theme`.
...
Możesz także definiować agentów przy użyciu plików markdown w `~/.config/HopCoderX/agents/` lub `.HopCoderX/agents/`. [Dowiedz się więcej o agentach](/docs/agents).
...
Agent domyślny musi być agentem typu `primary` (nie `subagent`). Może to być agent wbudowany, taki jak `"build"` lub `"plan"`, albo zdefiniowany przez Ciebie [agent niestandardowy](/docs/agents). Jeśli agent nie istnieje lub jest subagentem, HopCoderX powróci do `"build"` z ostrzeżeniem.

Dotyczy to wszystkich interfejsów: TUI, CLI (`HopCoderX run`), aplikacji desktopowej i GitHub Actions.
...
Możesz także definiować polecenia przy użyciu plików Markdown w `~/.config/HopCoderX/commands/` lub `.HopCoderX/commands/`. [Dowiedz się więcej o poleceniach](/docs/commands).
...
HopCoderX może automatycznie pobierać nowe wersje. Możesz to kontrolować za pomocą opcji `autoupdate`.
...
Działa to tylko wtedy, gdy HopCoderX nie został zainstalowany przez menedżera pakietów, takiego jak Homebrew.
...
Domyślnie HopCoderX **zezwala na wszystkie działania** bez konieczności zatwierdzania. Możesz to zmienić za pomocą opcji `permission`.
...
[Wtyczki](/docs/plugins) rozszerzają HopCoderX o niestandardowe narzędzia, hooki i integracje.

Umieść pliki wtyczek w `.HopCoderX/plugins/` lub `~/.config/HopCoderX/plugins/`. Możesz także załadować wtyczkę z npm za pomocą opcji `plugin`.
...
Jest to przydatne, gdy chcesz ograniczyć użycie HopCoderX tylko do określonych dostawców.
