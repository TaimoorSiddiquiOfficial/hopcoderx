---
title: Serwer
description: Interakcja z serwerem HopCoderX za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `HopCoderX serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient HopCoderX.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `HopCoderX.local` |
...
Ustaw `HOPCODERX_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `HopCoderX` lub ustaw `HOPCODERX_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `HopCoderX serve`, jak i `HopCoderX web`.
...
Kiedy uruchomisz `HopCoderX`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera HopCoderX do programowej interakcji z HopCoderX.
:::

Ta architektura umożliwia obsługę wielu klientów przez HopCoderX i programową interakcję z kodem otwartym.

Możesz uruchomić `HopCoderX serve`, aby uruchomić samodzielny serwer. Jeśli masz
HopCoderX TUI działa, `HopCoderX serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki HopCoderX [IDE](/docs/ide).
...
Serwer HopCoderX udostępnia następujące interfejsy API.
