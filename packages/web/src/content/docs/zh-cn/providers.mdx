...
HopCoderX Bdr 是由 HopCoderX 团队提供的模型列表，这些模型已经过测试和验证，能够与 HopCoderX 良好配合使用。[了解更多](/docs/zen)。
...
:::note[合规说明]
HopCoderX 会使用一个小模型来执行部分 AI 任务，例如生成会话标题。默认情况下使用由 Zen 托管的 gpt-5-nano。如果你需要让 HopCoderX 仅使用你自己的 GitLab 托管实例，请在 `HopCoderX.json` 文件中添加以下内容。同时建议禁用会话共享。
...
HopCoderX Bdr 是由 HopCoderX 团队提供的经过测试和验证的模型列表。[了解更多](/docs/zen)。
