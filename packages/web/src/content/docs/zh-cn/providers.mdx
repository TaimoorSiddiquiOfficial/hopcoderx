...
HopCoderX Zen 是由 HopCoderX 团队提供的模型列表，这些模型已经过测试和验证，能够与 HopCoderX 良好配合使用。[了解更多](/docs/zen)。

:::tip
如果你是新用户，我们建议从 HopCoderX Zen 开始。
:::

1. 在 TUI 中执行 `/connect` 命令，选择 HopCoderX，然后前往 [HopCoderX.ai/auth](https://HopCoderX.ai/auth)。
...
HopCoderX Zen 是由 HopCoderX 团队提供的经过测试和验证的模型列表。[了解更多](/docs/zen)。

1. 登录 **<a href={console}>HopCoderX Zen</a>** 并点击 **Create API Key**。

2. 执行 `/connect` 命令并搜索 **HopCoderX Zen**。
