...
Sie können externe Tools zu HopCoderX hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. HopCoderX unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [HopCoderX Config](https://hopcoderx.dev/docs/config/) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.
...
Organisationen können über ihren `.well-known/HopCoderX`-Endpunkt Standard-MCP-Server bereitstellen. Diese Server sind möglicherweise standardmäßig deaktiviert, sodass Benutzer sich für den Server entscheiden können, die sie benötigen.
...
HopCoderX übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird HopCoderX Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von HopCoderX beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `HopCoderX mcp auth <server-name>` verwenden.
...
Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert HopCoderX die Token sicher in `~/.local/share/HopCoderX/mcp-auth.json`.
...
Ihre MCPs sind neben integrierten Tools auch als Tools in HopCoderX verfügbar. Sie können sie also wie jedes andere Tool über die HopCoderX-Konfiguration verwalten.
...
Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und HopCoderX mit Ihrem Sentry-Konto zu verbinden.
