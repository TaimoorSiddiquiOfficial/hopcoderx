---
title: Server
description: Nutze den HopCoderX-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `HopCoderX serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein HopCoderX-Client nutzen kann.
...
Setze `HOPCODERX_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `HopCoderX`, kann aber mit `HOPCODERX_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `HopCoderX serve` und `HopCoderX web`.
...
Wenn du `HopCoderX` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den HopCoderX-Server, um HopCoderX programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `HopCoderX serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `HopCoderX serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die HopCoderX-[IDE](/docs/ide)-Plugins.
...
Der HopCoderX-Server stellt folgende APIs bereit.
