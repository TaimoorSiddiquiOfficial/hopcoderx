---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um HopCoderX zu erweitern.
---

Mit Plugins können Sie HopCoderX erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von HopCoderX zu ändern.
...
- `.HopCoderX/plugins/` – Plugins auf Projektebene
- `~/.config/HopCoderX/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/HopCoderX/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/HopCoderX/HopCoderX.json`)
2. Projektkonfiguration (`HopCoderX.json`)
3. Globales Plugin-Verzeichnis (`~/.config/HopCoderX/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.HopCoderX/plugins/`)
...
HopCoderX führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein HopCoderX-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie HopCoderX erweitern können.
...
:::note
Wenn Sie die HopCoderX-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass HopCoderX `.env`-Dateien liest:
...
Plugins können HopCoderX auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das HopCoderX aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für HopCoderX zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://hopcoderx.dev/docs/sdk).
