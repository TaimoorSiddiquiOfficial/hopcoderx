...
Wenn HopCoderX Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `HopCoderX --log-level DEBUG`.

---

## Speicher

HopCoderX speichert Sitzungs- und App-Daten auf der Festplatte unter:
...
HopCoderX Desktop startet im Hintergrund einen lokalen HopCoderX-Server (`HopCoderX-cli`-Sidecar).
...
- Nur macOS: `HopCoderX`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
Wenn du Probleme mit HopCoderX hast:
