...
Agent Skills erlauben HopCoderX, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
HopCoderX sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft HopCoderX vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.HopCoderX/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/HopCoderX/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.HopCoderX/skills/git-release/SKILL.md` zum Beispiel so:
...
HopCoderX listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `HopCoderX.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `HopCoderX.json`):
