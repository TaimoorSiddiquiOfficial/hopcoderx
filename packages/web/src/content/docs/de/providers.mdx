---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in HopCoderX.
---

import config from "../../../../config.mjs"
export const console = config.console

HopCoderX verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer HopCoderX-Konfiguration.
...
in `~/.local/share/HopCoderX/auth.json`.

---

### Konfiguration

Sie können die Anbieter über den Abschnitt `provider` in Ihrem HopCoderX anpassen
config.
...
HopCoderX Bdr ist eine Liste von Modellen, die vom HopCoderX-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit HopCoderX funktioniert. [Learn more](/docs/bdr).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit HopCoderX Bdr zu beginnen.
:::

1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie HopCoderX aus und gehen Sie zu [hopcoderx.dev/auth](https://hopcoderx.dev/auth).
...
Es funktioniert wie jeder andere Anbieter in HopCoderX und ist völlig optional.
...
So verwenden Sie Amazon Bedrock mit HopCoderX:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie HopCoderX ausführen:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `HopCoderX.json`:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in HopCoderX wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit HopCoderX ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre HopCoderX-Konfiguration hinzufügen.
...
um das Token im HopCoderX-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
HopCoderX verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Bdr gehostet. Zum Sperren von HopCoderX
Um nur Ihre eigene von GitLab gehostete Instanz zu verwenden, fügen Sie Folgendes zu Ihrer hinzu
`HopCoderX.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
Weitere Dokumentation auf der [HopCoderX-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth)-Homepage.

##### Konfiguration

Anpassen über `HopCoderX.json`:
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit HopCoderX:
...
So verwenden Sie Google Vertex AI mit HopCoderX:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie HopCoderX ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über HopCoderX konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [HopCoderX-helicone-session](https://github.com/H2Shami/HopCoderX-helicone-session)-Plugin, um jede HopCoderX-Konversation automatisch als Sitzung in Helicone zu protokollieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede HopCoderX-Konversation als separate Sitzung aufgeführt.
...
Sie können HopCoderX für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können HopCoderX über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können HopCoderX über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für HopCoderX konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/HopCoderX).
:::
...
So verwenden Sie Ollama Cloud mit HopCoderX:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in HopCoderX.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in HopCoderX verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
HopCoderX Bdr ist eine Liste getesteter und verifizierter Modelle, die vom HopCoderX-Team bereitgestellt werden. [Learn more](/docs/bdr).

1. Melden Sie sich bei **<a href={console}>HopCoderX Bdr</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **HopCoderX Bdr**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren HopCoderX API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre HopCoderX-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre HopCoderX-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit HopCoderX:
...
Sie können Modelle auch über Ihre HopCoderX-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit HopCoderX verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
4. Erstellen oder aktualisieren Sie Ihre `HopCoderX.json`-Datei in Ihrem Projektverzeichnis:
...
Mithilfe der `limit`-Felder kann HopCoderX erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `HopCoderX auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die HopCoderX-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer HopCoderX-Konfiguration übereinstimmt.
