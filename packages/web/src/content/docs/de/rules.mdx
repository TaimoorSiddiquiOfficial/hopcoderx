---
title: Regeln
description: Lege eigene Anweisungen fuer HopCoderX fest.
---

Du kannst HopCoderX ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in HopCoderX den Befehl `/init` aus.
...
So kann HopCoderX sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht HopCoderX sie zu erweitern.
...
HopCoderX kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/HopCoderX/AGENTS.md` speichern.
Diese gelten in allen HopCoderX-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt HopCoderX dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/HopCoderX/AGENTS.md` existiert)
...
Beim Start sucht HopCoderX in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/HopCoderX/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `HopCoderX.json` oder in `~/.config/HopCoderX/HopCoderX.json` angeben.
...
HopCoderX parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit HopCoderX.json

Empfohlen ist das `instructions`-Feld in der `HopCoderX.json`:
...
Du kannst HopCoderX explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `HopCoderX.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
