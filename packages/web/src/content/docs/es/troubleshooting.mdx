...
Para depurar problemas con HopCoderX, comience verificando los registros y los datos locales que almacena en el disco.
...
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\HopCoderX\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `HopCoderX --log-level DEBUG`.

---

## Almacenamiento

HopCoderX almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/HopCoderX/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\HopCoderX`
...
HopCoderX Desktop ejecuta un servidor HopCoderX local (el sidecar `HopCoderX-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `HopCoderX` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/HopCoderX/HopCoderX.jsonc` (o `~/.config/HopCoderX/HopCoderX.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/HopCoderX/HopCoderX.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\HopCoderX\HopCoderX.jsonc`
...
HopCoderX también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/HopCoderX/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\HopCoderX\plugins`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que HopCoderX pueda reconstruirlo.

1. Salga de HopCoderX Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/HopCoderX`
- **Linux**: eliminar `~/.cache/HopCoderX` (o ejecutar `rm -rf ~/.cache/HopCoderX`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\HopCoderX`

3. Reinicie el escritorio HopCoderX.

---

### Solucionar problemas de conexión del servidor

HopCoderX Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `HopCoderX.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `HOPCODERX_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `HOPCODERX_PORT` (o elija un puerto libre) y reinicie.
...
En Windows, el escritorio HopCoderX requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de HopCoderX.

---

### Notificaciones que no se muestran

HopCoderX Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para HopCoderX en la configuración de su sistema operativo, y
...
1. Salga del escritorio HopCoderX.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio HopCoderX):

- `HopCoderX.settings.dat` (URL del servidor predeterminado de escritorio)
- `HopCoderX.global.dat` y `HopCoderX.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
Si tiene problemas con HopCoderX:
...
2. **Únete a nuestro Discord**
   ...
   [**hopcoderx.dev/discord**](https://hopcoderx.dev/discord)
...
3. Asegúrese de tener la última versión con `HopCoderX upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `HopCoderX models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\HopCoderX`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. HopCoderX instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\HopCoderX`

2. Reinicie HopCoderX para reinstalar los paquetes del proveedor más recientes.

Esto obligará a HopCoderX a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
HopCoderX detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
