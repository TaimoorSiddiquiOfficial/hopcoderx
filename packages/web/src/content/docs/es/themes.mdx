...
Con HopCoderX puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, HopCoderX usa nuestro propio tema `HopCoderX`.
...
HopCoderX viene con varios temas integrados.
...
- Quiere que HopCoderX coincida con la apariencia de su terminal
...
HopCoderX admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/HopCoderX/themes/*.json` o `$XDG_CONFIG_HOME/HopCoderX/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.HopCoderX/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.HopCoderX/themes/*.json`
