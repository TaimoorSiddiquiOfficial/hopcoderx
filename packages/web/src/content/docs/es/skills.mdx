...
Las habilidades del agente permiten a HopCoderX descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
HopCoderX busca estas ubicaciones:

- Configuración del proyecto: `.HopCoderX/skills/<name>/SKILL.md`
- Configuración global: `~/.config/HopCoderX/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, HopCoderX sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.HopCoderX/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/HopCoderX/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.HopCoderX/skills/git-release/SKILL.md` así:
...
HopCoderX enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `HopCoderX.json`:
...
**Para agentes integrados** (en `HopCoderX.json`):
