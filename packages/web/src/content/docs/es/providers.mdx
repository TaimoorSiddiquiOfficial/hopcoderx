---
title: Proveedores
description: Usando cualquier proveedor LLM en HopCoderX.
---

import config from "../../../../config.mjs"
export const console = config.console

HopCoderX usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración HopCoderX.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu HopCoderX
configuración.
...
HopCoderX Bdr es una lista de modelos proporcionados por el equipo HopCoderX que han sido
probado y verificado para funcionar bien con HopCoderX. [Más información](/docs/bdr).

:::tip
Si eres nuevo, te recomendamos comenzar con HopCoderX Bdr.
:::

1. Ejecute el comando `/connect` en TUI, seleccione HopCoderX y diríjase a [hopcoderx.dev/auth](https://hopcoderx.dev/auth).
...
Funciona como cualquier otro proveedor en HopCoderX y su uso es completamente opcional.
...
Para usar Amazon Bedrock con HopCoderX:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Establezca una de estas variables de entorno mientras ejecuta HopCoderX:
   ...
   Para una configuración persistente o específica del proyecto, utilice `HopCoderX.json`:
...
:::info
El uso de su suscripción Claude Pro/Max en HopCoderX no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que HopCoderX funcione correctamente.
   :::
...
5. Ejecute el comando `/models` para seleccionar un modelo.
   ...
   También puede agregar modelos a través de su configuración HopCoderX.
...
2. Elija su método de autenticación:
   ...
   3. Nombre: `HopCoderX`, Alcances: `api`
...
para almacenar el token en el almacenamiento de autenticación HopCoderX.
:::

##### Autohospedado GitLab

:::note[nota de cumplimiento]
HopCoderX utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Bdr. Para bloquear HopCoderX
Para usar solo su propia instancia alojada en GitLab, agregue lo siguiente a su
Archivo `HopCoderX.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Más documentación en la página de inicio de [HopCoderX-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth).

##### Configuración

Personalizar a través de `HopCoderX.json`:
...
Para utilizar su suscripción GitHub Copilot con HopCoderX:
...
Para utilizar Google Vertex AI con HopCoderX:
...
2. Establezca las variables de entorno requeridas:
   ...
   Configúrelos mientras ejecuta HopCoderX.
...
En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de HopCoderX, siempre podrá configurarlo usted mismo.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación HopCoderX enumerada como una sesión separada.
...
Puede configurar HopCoderX para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puede configurar HopCoderX para usar modelos locales a través de LM Studio.
...
Puedes configurar HopCoderX para usar modelos locales a través de Ollama.

:::tip
Ollama puede configurarse automáticamente para HopCoderX. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/HopCoderX) para obtener más detalles.
:::
...
Para usar Ollama Cloud con HopCoderX:
...
3. Copie la clave API para usarla en HopCoderX.
...
6. **Importante**: Antes de usar modelos en la nube en HopCoderX, debe obtener la información del modelo localmente:
...
HopCoderX Bdr es una lista de modelos probados y verificados proporcionada por el equipo HopCoderX. [Más información](/docs/bdr).

1. Inicie sesión en **<a href={console}>HopCoderX Bdr</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **HopCoderX Bdr**.

   ```txt
   /connect
   ```

3. Ingrese su clave HopCoderX API.
...
4. Muchos modelos de OpenRouter están precargados de forma predeterminada, ejecute el comando `/models` para seleccionar el que desee.
   ...
   También puede agregar modelos adicionales a través de su configuración HopCoderX.
...
5. También puedes personalizarlos a través de tu configuración HopCoderX. A continuación se muestra un ejemplo de cómo especificar un proveedor.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con HopCoderX:
...
También puede personalizar modelos a través de su configuración HopCoderX. A continuación se muestra un ejemplo de cómo especificar el orden de enrutamiento del proveedor.
...
:::tip
Puede utilizar cualquier proveedor compatible con OpenAI con HopCoderX. La mayoría de los proveedores de IA modernos ofrecen API compatibles con OpenAI.
:::
...
4. Cree o actualice su archivo `HopCoderX.json` en el directorio de su proyecto:
...
Los campos `limit` le permiten a HopCoderX comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
1. **Verifique la configuración de autenticación**: Ejecute `HopCoderX auth list` para ver si las credenciales
...
2. Para proveedores personalizados, verifique la configuración HopCoderX y:
   - Asegúrese de que el ID del proveedor utilizado en el comando `/connect` coincida con el ID en su configuración HopCoderX.
