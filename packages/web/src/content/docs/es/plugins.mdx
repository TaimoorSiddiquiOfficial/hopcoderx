---
title: Complementos
description: Escriba sus propios complementos para extender HopCoderX.
---

Los complementos le permiten extender HopCoderX al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de HopCoderX.
...
- `.HopCoderX/plugins/` - Complementos a nivel de proyecto
- `~/.config/HopCoderX/plugins/` - Complementos globales
...
```json title="HopCoderX.json"
{
  ...
  "plugin": ["HopCoderX-helicone-session", "HopCoderX-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/HopCoderX/node_modules/`.
...
1. Configuración global (`~/.config/HopCoderX/HopCoderX.json`)
2. Configuración del proyecto (`HopCoderX.json`)
3. Directorio global de complementos (`~/.config/HopCoderX/plugins/`)
4. Directorio de complementos del proyecto (`.HopCoderX/plugins/`)
...
```json title=".HopCoderX/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
HopCoderX ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".HopCoderX/plugins/my-plugin.ts"
...
```js title=".HopCoderX/plugins/example.js"
...
- `client`: Un cliente SDK HopCoderX para interactuar con la IA.
...
import type { Plugin } from "@HopCoderX-ai/plugin"
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar HopCoderX.
...
```js title=".HopCoderX/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "HopCoderX"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio HopCoderX, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que HopCoderX lea archivos `.env`:

```javascript title=".HopCoderX/plugins/env-protection.js"
...
```javascript title=".HopCoderX/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a HopCoderX:

```ts title=".HopCoderX/plugins/custom-tools.ts"
import { type Plugin, tool } from "@HopCoderX-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que HopCoderX puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para HopCoderX junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://hopcoderx.dev/docs/sdk) para obtener más detalles.
...
```ts title=".HopCoderX/plugins/compaction.ts"
import type { Plugin } from "@HopCoderX-ai/plugin"
...
```ts title=".HopCoderX/plugins/custom-compaction.ts"
import type { Plugin } from "@HopCoderX-ai/plugin"
