---
title: Servidor
description: Interactuar con el servidor HopCoderX a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `HopCoderX serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente HopCoderX puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `HopCoderX.local` |
...
Configure `HOPCODERX_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `HopCoderX`, o configure `HOPCODERX_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `HopCoderX serve` como a `HopCoderX web`.
...
Cuando ejecuta `HopCoderX`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor HopCoderX para interactuar con HopCoderX mediante programación.
:::

Esta arquitectura permite que HopCoderX admita múltiples clientes y le permite interactuar con HopCoderX mediante programación.

Puede ejecutar `HopCoderX serve` para iniciar un servidor independiente. Si tienes el
HopCoderX TUI ejecutándose, `HopCoderX serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos HopCoderX [IDE](/docs/ide).
...
El servidor HopCoderX expone las siguientes API.
