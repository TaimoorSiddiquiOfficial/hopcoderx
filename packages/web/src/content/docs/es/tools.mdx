...
Las herramientas permiten que LLM realice acciones en su código base. HopCoderX viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en HopCoderX.
...
:::note
Esta herramienta solo está disponible cuando `HOPCODERX_EXPERIMENTAL_LSP_TOOL=true` (o `HOPCODERX_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor HopCoderX o cuando la variable de entorno `HOPCODERX_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar HopCoderX:

```bash
HOPCODERX_ENABLE_EXA=1 HopCoderX
```
