...
Com o HopCoderX, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o HopCoderX usa nosso próprio tema `HopCoderX`.
...
O HopCoderX vem com vários temas embutidos.
...
- Querem que o HopCoderX corresponda à aparência do seu terminal
...
O HopCoderX suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/HopCoderX/themes/*.json` ou `$XDG_CONFIG_HOME/HopCoderX/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.HopCoderX/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.HopCoderX/themes/*.json`
