---
title: Plugins
description: Escreva seus próprios plugins para estender o HopCoderX.
---

Plugins permitem que você estenda o HopCoderX conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do HopCoderX.
...
- `.HopCoderX/plugins/` - Plugins em nível de projeto
- `~/.config/HopCoderX/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/HopCoderX/node_modules/`.
...
1. Configuração global (`~/.config/HopCoderX/HopCoderX.json`)
2. Configuração do projeto (`HopCoderX.json`)
3. Diretório de plugins global (`~/.config/HopCoderX/plugins/`)
4. Diretório de plugins do projeto (`.HopCoderX/plugins/`)
...
O HopCoderX executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do HopCoderX para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o HopCoderX.
...
:::note
Se você estiver usando o aplicativo desktop HopCoderX, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o HopCoderX de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao HopCoderX:
...
O helper `tool` cria uma ferramenta personalizada que o HopCoderX pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o HopCoderX junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://hopcoderx.dev/docs/sdk) para detalhes.
