...
Existem dois tipos de agentes no HopCoderX; agentes primários e subagentes.
...
HopCoderX vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
HopCoderX vem com dois subagentes integrados, **General** e **Explore**. Vamos ver isso abaixo.

---

## Integrados

HopCoderX vem com dois agentes primários integrados e dois subagentes integrados.
...
Configure os agentes em seu arquivo de configuração `HopCoderX.json`:
...
Se nenhuma temperatura for especificada, o HopCoderX usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do HopCoderX quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do HopCoderX usa o formato `provider/model-id`. Por exemplo, se você estiver usando [HopCoderX Bdr](/docs/zen), você usaria `HopCoderX/gpt-5.1-codex` para GPT 5.1 Codex.
...
:::tip
Execute `HopCoderX models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/TaimoorSiddiquiOfficial/hopcoderx).
:::
