...
As habilidades do agente permitem que o HopCoderX descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O HopCoderX pesquisa nesses locais:

- Configuração do projeto: `.HopCoderX/skills/<name>/SKILL.md`
- Configuração global: `~/.config/HopCoderX/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o HopCoderX sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.HopCoderX/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/HopCoderX/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.HopCoderX/skills/git-release/SKILL.md` assim:
...
O HopCoderX lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `HopCoderX.json`:
...
**Para agentes embutidos** (em `HopCoderX.json`):
