---
title: Servidor
description: Interaja com o servidor HopCoderX via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `HopCoderX serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente HopCoderX pode usar.
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `HopCoderX.local` |
...
Defina `HOPCODERX_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `HopCoderX`, ou defina `HOPCODERX_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `HopCoderX serve` quanto ao `HopCoderX web`.

```bash
HOPCODERX_SERVER_PASSWORD=your-password HopCoderX serve
```
...
Quando você executa `HopCoderX`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor HopCoderX para interagir com o HopCoderX programaticamente.
:::

Essa arquitetura permite que o HopCoderX suporte múltiplos clientes e permite que você interaja com o HopCoderX programaticamente.

Você pode executar `HopCoderX serve` para iniciar um servidor autônomo. Se você tiver o TUI do HopCoderX em execução, `HopCoderX serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do HopCoderX [IDE](/docs/ide).
...
O servidor HopCoderX expõe as seguintes APIs.
