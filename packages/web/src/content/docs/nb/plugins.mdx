...
description: Skriv dine egne programtillegg for å utvide HopCoderX.
---

Plugins lar deg utvide HopCoderX ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til HopCoderX.
...
- `.HopCoderX/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/HopCoderX/node_modules/`.
...
1. Global konfigurasjon (`~/.config/HopCoderX/HopCoderX.json`)
2. Prosjektkonfigurasjon (`HopCoderX.json`)
3. Global plugin-katalog (`~/.config/HopCoderX/plugins/`)
4. Prosjektpluginkatalog (`.HopCoderX/plugins/`)
...
HopCoderX kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En HopCoderX SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide HopCoderX.
...
:::note
Hvis du bruker HopCoderX-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre HopCoderX fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til HopCoderX:
...
`tool`-hjelperen lager et tilpasset verktøy som HopCoderX kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for HopCoderX sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://hopcoderx.dev/docs/sdk) for detaljer.
