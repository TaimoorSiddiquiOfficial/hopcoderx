---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i HopCoderX.
---

import config from "../../../../config.mjs"
export const console = config.console

HopCoderX bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i HopCoderX-konfigurasjonen.
...
i `~/.local/share/HopCoderX/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i HopCoderX
konfig.
...
HopCoderX Bdr er en liste over modeller levert av HopCoderX-teamet som har vært
testet og verifisert for å fungere godt med HopCoderX. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med HopCoderX Bdr.
:::

1. Kjør kommandoen `/connect` i TUI, velg HopCoderX og gå til [hopcoderx.dev/auth](https://hopcoderx.dev/auth).
...
Det fungerer som alle andre leverandører i HopCoderX og er helt valgfritt å bruke.
...
Slik bruker du Amazon Bedrock med HopCoderX:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører HopCoderX:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `HopCoderX.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i HopCoderX støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at HopCoderX skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom HopCoderX-konfigurasjonen.
...
for å lagre token i HopCoderX auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
HopCoderX bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse HopCoderX
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`HopCoderX.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [HopCoderX-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `HopCoderX.json`:
...
Slik bruker du GitHub Copilot-abonnementet med HopCoderX:
...
Slik bruker du Google Vertex AI med HopCoderX:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører HopCoderX.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom HopCoderX, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [HopCoderX-helicone-session](https://github.com/H2Shami/HopCoderX-helicone-session) plugin for automatisk å logge hver HopCoderX-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver HopCoderX-samtale oppført som en separat økt.
...
Du kan konfigurere HopCoderX for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere HopCoderX for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere HopCoderX for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for HopCoderX. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/HopCoderX) for detaljer.
:::
...
Slik bruker du Ollama Cloud med HopCoderX:
...
3. Kopier API-nøkkelen for bruk i HopCoderX.
...
6. **Viktig**: Før du bruker skymodeller i HopCoderX, må du hente modellinformasjonen lokalt:
...
HopCoderX Bdr er en liste over testede og verifiserte modeller levert av HopCoderX-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>HopCoderX Bdr</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **HopCoderX Bdr**.

   ```txt
   /connect
   ```

3. Skriv inn HopCoderX API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom HopCoderX-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom HopCoderX-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med HopCoderX:
...
Du kan også tilpasse modeller gjennom HopCoderX-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med HopCoderX. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `HopCoderX.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar HopCoderX forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `HopCoderX auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk HopCoderX-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i HopCoderX-konfigurasjonen.
