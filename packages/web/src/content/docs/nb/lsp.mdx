---
title: LSP-servere
description: HopCoderX integreres med LSP-serverne dine.
---

HopCoderX integreres med din Language Server Protocol (LSP) for å hjelpe LLM med å samhandle med kodebasen din. Den bruker diagnostikk for å gi tilbakemelding til LLM.

---

## Innebygde servere

HopCoderX kommer med flere innebygde LSP-servere for populære språk:
...
:::note
Du kan deaktivere automatiske LSP-servernedlastinger ved å sette miljøvariabelen `HOPCODERX_DISABLE_LSP_DOWNLOAD` til `true`.
:::
...
Når HopCoderX åpner en fil, gjør den:
...
Du kan tilpasse LSP-servere gjennom `lsp`-delen i HopCoderX-konfigurasjonen.
