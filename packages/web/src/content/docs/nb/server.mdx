---
title: Tjener
description: Samhandle med HopCoderX-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `HopCoderX serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en HopCoderX-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `HopCoderX.local` |
...
Sett `HOPCODERX_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `HopCoderX` som standard, eller sett `HOPCODERX_SERVER_USERNAME` for å overstyre det. Dette gjelder både `HopCoderX serve` og `HopCoderX web`.
...
Når du kjører `HopCoderX` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk HopCoderX-serveren til å samhandle med HopCoderX programmatisk.
:::

Denne arkitekturen lar HopCoderX støtte flere klienter og lar deg samhandle med HopCoderX programmatisk.

Du kan kjøre `HopCoderX serve` for å starte en frittstående server. Hvis du har
HopCoderX TUI kjører, vil `HopCoderX serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av HopCoderX [IDE](/docs/ide) plugins.
...
HopCoderX-serveren viser følgende APIer.
