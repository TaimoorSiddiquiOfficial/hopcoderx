...
Du kan legge til eksterne verktøy i HopCoderX ved å bruke _Model Context Protocol_ eller MCP. HopCoderX støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [HopCoderX Config](https://hopcoderx.dev/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="HopCoderX.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/HopCoderX`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
```jsonc title="HopCoderX.jsonc" {15}
{
  "$schema": "https://hopcoderx.dev/config.json",
  "mcp": {
    "my-local-mcp-server": {
...
HopCoderX håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil HopCoderX:
...
Hvis serveren krever autentisering, vil HopCoderX be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `HopCoderX mcp auth <server-name>`.
...
```json title="HopCoderX.json" {7-11}
{
  "$schema": "https://hopcoderx.dev/config.json",
  "mcp": {
    "my-oauth-server": {
...
Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil HopCoderX lagre tokens sikkert i `~/.local/share/HopCoderX/mcp-auth.json`.
...
MCP-ene dine er tilgjengelige som verktøy i HopCoderX, sammen med innebygde verktøy. Så du kan administrere dem gjennom HopCoderX-konfigurasjonen som alle andre verktøy.
...
```json title="HopCoderX.json" {11, 14-18}
{
  "$schema": "https://hopcoderx.dev/config.json",
  "mcp": {
    "my-mcp": {
...
Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble HopCoderX til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver HopCoderX-samtale oppført som en separat økt.
