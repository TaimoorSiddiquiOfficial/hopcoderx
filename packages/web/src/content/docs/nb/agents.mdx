...
Det er to typer agenter i HopCoderX; primære agenter og underagenter.
...
HopCoderX kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
HopCoderX kommer med to innebygde underagenter, **General** og **Explore**. Vi skal se på dette nedenfor.

---

## Innebygd

HopCoderX kommer med to innebygde primære agenter og to innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `HopCoderX.json`:
...
- Globalt: `~/.config/HopCoderX/agents/`
- Per prosjekt: `.HopCoderX/agents/`
...
Hvis ingen temperatur er spesifisert, bruker HopCoderX modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale HopCoderX-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i HopCoderX-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [HopCoderX Bdr](/docs/zen), vil du bruke `HopCoderX/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `HopCoderX models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/TaimoorSiddiquiOfficial/hopcoderx).
:::
