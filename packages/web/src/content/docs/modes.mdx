...
:::caution
Modes are now configured through the `agent` option in the HopCoderX config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in HopCoderX allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the HopCoderX config.
...
HopCoderX comes with two built-in modes.
...
- `edit` - Cannot modify existing files, except for files located at `.HopCoderX/plans/*.md` to detail the plan itself
...
Configure modes in your `HopCoderX.json` config file:
...
both the global HopCoderX config and the project specific config.
...
Create mode files in `.HopCoderX/modes/` for project-specific modes or `~/.config/HopCoderX/modes/` for global modes:
