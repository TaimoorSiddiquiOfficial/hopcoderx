...
Create `.HopCoderX/commands/test.md`:
...
You can add custom commands through the HopCoderX config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your HopCoderX [config](/docs/config):
...
- Per-project: `.HopCoderX/commands/`
...
HopCoderX includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
