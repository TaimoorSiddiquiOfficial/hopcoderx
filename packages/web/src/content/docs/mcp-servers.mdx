...
You can add external tools to HopCoderX using the _Model Context Protocol_, or MCP. HopCoderX supports both local and remote servers.
...
You can define MCP servers in your [HopCoderX Config](https://hopcoderx.dev/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/HopCoderX` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
If you prefer curated entries instead of writing the config by hand, use the registry install flow:

```bash
HopCoderX mcp install powershell-mcp
```

`powershell-mcp` and `windows-mcp` solve different problems:

- Use `powershell-mcp` for a persistent PowerShell / CLI session powered by `pwsh` and the `PowerShell.MCP` module.
- Use `windows-mcp` for Windows desktop and UI automation such as screenshots, mouse/keyboard control, and UI tree inspection.
...
HopCoderX automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, HopCoderX will:
...
If the server requires authentication, HopCoderX will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `HopCoderX mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, HopCoderX will store the tokens securely in `~/.local/share/HopCoderX/mcp-auth.json`.
...
Your MCPs are available as tools in HopCoderX, alongside built-in tools. So you can manage them through the HopCoderX config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect HopCoderX to your Sentry account.
