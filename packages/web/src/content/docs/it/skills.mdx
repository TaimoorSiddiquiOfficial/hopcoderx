...
Le skill degli agenti permettono a HopCoderX di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
HopCoderX cerca in queste posizioni:

- Config di progetto: `.HopCoderX/skills/<name>/SKILL.md`
- Config globale: `~/.config/HopCoderX/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, HopCoderX risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.HopCoderX/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/HopCoderX/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.HopCoderX/skills/git-release/SKILL.md` cosi':
...
HopCoderX elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `HopCoderX.json`:
...
**Per agenti integrati** (in `HopCoderX.json`):
