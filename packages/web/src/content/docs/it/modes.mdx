...
:::caution
Le modalita ora si configurano tramite l'opzione `agent` nella configurazione di HopCoderX. L'opzione
`mode` e ora deprecata. [Scopri di piu](/docs/agents).
:::

Le modalita in HopCoderX ti permettono di personalizzare comportamento, strumenti e prompt per casi d'uso diversi.

Include due modalita integrate: **build** e **plan**. Puoi personalizzarle oppure configurarne di tue tramite la configurazione di HopCoderX.
...
HopCoderX include due modalita integrate.
...
- `edit` - Non puo modificare file esistenti, tranne i file in `.HopCoderX/plans/*.md` per dettagliare il piano
...
Configura le modalita nel file di configurazione `HopCoderX.json`:
...
- Globale: `~/.config/HopCoderX/modes/`
- Progetto: `.HopCoderX/modes/`
...
Se non viene specificata alcuna temperatura, HopCoderX usa i default specifici del modello (tipicamente 0 per la maggior parte dei modelli, 0.55 per i modelli Qwen).
...
sia per la configurazione globale di HopCoderX sia per quella specifica del progetto.
...
Crea file di modalita in `.HopCoderX/modes/` per modalita specifiche del progetto oppure in `~/.config/HopCoderX/modes/` per modalita globali:
