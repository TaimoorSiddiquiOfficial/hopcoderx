...
In HopCoderX esistono due tipi di agenti: agenti primari e subagenti.
...
HopCoderX include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
HopCoderX include due subagenti integrati: **General** e **Explore**. Li vediamo sotto.

---

## Integrati

HopCoderX include due agenti primari integrati e due subagenti integrati.
...
Configura gli agenti nel file `HopCoderX.json`:
...
- Globale: `~/.config/HopCoderX/agents/`
- Per progetto: `.HopCoderX/agents/`
...
Se non specifichi una temperature, HopCoderX usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di HopCoderX sia per la config specifica del progetto.
...
L'ID modello nella configurazione HopCoderX usa il formato `provider/model-id`. Per esempio, se stai usando [HopCoderX Bdr](/docs/bdr), useresti `HopCoderX/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `HopCoderX models` per vedere la lista dei modelli disponibili.
:::
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/TaimoorSiddiquiOfficial/hopcoderx).
:::
