---
title: Server
description: Interagisci con il server HopCoderX via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `HopCoderX serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client HopCoderX.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `HopCoderX.local` |
...
Imposta `HOPCODERX_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `HopCoderX`, oppure imposta `HOPCODERX_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `HopCoderX serve` sia per `HopCoderX web`.
...
Quando esegui `HopCoderX` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server HopCoderX per interagire con HopCoderX in modo programmatico.
:::

Questa architettura permette a HopCoderX di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `HopCoderX serve` per avviare un server standalone. Se la TUI di HopCoderX e gia in esecuzione, `HopCoderX serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di HopCoderX.
...
Il server HopCoderX espone le seguenti API.
