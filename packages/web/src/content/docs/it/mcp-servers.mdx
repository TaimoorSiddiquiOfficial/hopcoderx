...
Puoi aggiungere strumenti esterni a HopCoderX usando il _Model Context Protocol_, o MCP. HopCoderX supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [HopCoderX Config](https://hopcoderx.dev/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/HopCoderX`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
HopCoderX gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, HopCoderX:
...
Se il server richiede autenticazione, HopCoderX ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `HopCoderX mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, HopCoderX memorizzera' i token in modo sicuro in `~/.local/share/HopCoderX/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in HopCoderX insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione HopCoderX come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare HopCoderX al tuo account Sentry.
