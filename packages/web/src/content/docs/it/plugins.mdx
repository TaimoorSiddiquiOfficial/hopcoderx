---
title: Plugin
description: Scrivi plugin per estendere HopCoderX.
---

I plugin ti permettono di estendere HopCoderX agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalita', integrare servizi esterni o modificare il comportamento predefinito di HopCoderX.
...
- `.HopCoderX/plugins/` - plugin a livello progetto
- `~/.config/HopCoderX/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/HopCoderX/node_modules/`.
...
1. Config globale (`~/.config/HopCoderX/HopCoderX.json`)
2. Config di progetto (`HopCoderX.json`)
3. Directory plugin globale (`~/.config/HopCoderX/plugins/`)
4. Directory plugin di progetto (`.HopCoderX/plugins/`)
...
HopCoderX esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di HopCoderX per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere HopCoderX.
...
:::note
Se usi l'app desktop di HopCoderX, puo' inviare automaticamente notifiche di sistema quando una risposta e' pronta o quando una sessione va in errore.
:::
...
Impedisci a HopCoderX di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a HopCoderX:
...
L'helper `tool` crea uno strumento personalizzato che HopCoderX puo' chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in HopCoderX insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://hopcoderx.dev/docs/sdk) per i dettagli.
