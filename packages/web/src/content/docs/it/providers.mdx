---
title: Provider
description: Usa qualsiasi provider LLM in HopCoderX.
---

import config from "../../../../config.mjs"
export const console = config.console

HopCoderX usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di HopCoderX.
...
HopCoderX Bdr e una lista di modelli forniti dal team HopCoderX, testati e
verificati per funzionare bene con HopCoderX. [Scopri di piu](/docs/bdr).

:::tip
Se sei all'inizio, ti consigliamo di partire da HopCoderX Bdr.
:::

1. Esegui `/connect` nella TUI, seleziona HopCoderX e vai su [hopcoderx.dev/auth](https://hopcoderx.dev/auth).
...
Funziona come qualsiasi altro provider in HopCoderX ed e totalmente opzionale.
...
Per usare Amazon Bedrock con HopCoderX:
...
2. **Configura l'autenticazione** usando uno dei seguenti metodi:
   ...
   ***
   ...
   Imposta una di queste variabili d'ambiente mentre esegui HopCoderX:
   ...
   ***
   ...
   Per configurazione specifica del progetto o persistente, usa `HopCoderX.json`:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in HopCoderX non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
2. Vai su [Azure AI Foundry](https://ai.azure.com/) e fai il deploy di un modello.
   ...
   :::note
   Il nome del deployment deve corrispondere al nome del modello affinché HopCoderX funzioni correttamente.
   :::
...
5. Esegui il comando `/models` per selezionare un modello.
   ...
   Puoi anche aggiungere modelli tramite la tua configurazione di HopCoderX.
...
salvare il token nello storage di auth di HopCoderX.
:::

##### GitLab Self-Hosted

:::note[nota di compliance]
HopCoderX usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Bdr. Per bloccare HopCoderX
a usare solo la tua istanza GitLab self-hosted, aggiungi quanto segue al tuo
file `HopCoderX.json`. Si raccomanda anche di disabilitare la condivisione sessioni.
...
Maggior documentazione sulla homepage di [HopCoderX-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth).

##### Configurazione

Personalizza tramite `HopCoderX.json`:
...
Per usare il tuo abbonamento GitHub Copilot con HopCoderX:
...
Per usare Google Vertex AI con HopCoderX:
...
2. Imposta le variabili d'ambiente richieste:
   ...
   Impostale mentre esegui HopCoderX.
...
Nel caso vedessi una funzionalità o un modello di Helicone che non viene configurato automaticamente da HopCoderX, puoi sempre configurarlo tu stesso.
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [HopCoderX-helicone-session](https://github.com/H2Shami/HopCoderX-helicone-session) per loggare automaticamente ogni conversazione di HopCoderX come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di HopCoderX elencata come sessione separata.
...
Puoi configurare HopCoderX per usare modelli locali tramite l'utility llama-server di [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puoi configurare HopCoderX per usare modelli locali tramite LM Studio.
...
Puoi configurare HopCoderX per usare modelli locali tramite Ollama.

:::tip
Ollama può auto-configurarsi per HopCoderX. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/HopCoderX) per dettagli.
:::
...
Per usare Ollama Cloud con HopCoderX:
...
3. Copia la chiave API da usare in HopCoderX.
...
6. **Importante**: Prima di usare modelli cloud in HopCoderX, devi scaricare le informazioni del modello localmente:
...
HopCoderX Bdr è una lista di modelli testati e verificati forniti dal team HopCoderX. [Scopri di più](/docs/bdr).

1. Accedi a **<a href={console}>HopCoderX Bdr</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **HopCoderX Bdr**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di HopCoderX.
...
4. Molti modelli OpenRouter sono precaricati di default, esegui il comando `/models` per selezionare quello che vuoi.
   ...
   Puoi anche aggiungere modelli addizionali tramite la tua configurazione di HopCoderX.
...
5. Puoi anche personalizzarli tramite la configurazione di HopCoderX. Ecco un esempio di specifica di un provider
...
Per usare le [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) con HopCoderX:
...
Puoi anche personalizzare i modelli tramite la tua configurazione di HopCoderX. Ecco un esempio di come specificare l'ordine di routing dei provider.
...
:::tip
Puoi usare qualsiasi provider OpenAI-compatible con HopCoderX. La maggior parte dei provider AI moderni offre API OpenAI-compatible.
:::
...
4. Crea o aggiorna il tuo file `HopCoderX.json` nella directory del progetto:
...
I campi `limit` permettono a HopCoderX di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
...
1. **Controlla il setup auth**: Esegui `HopCoderX auth list` per vedere se le credenziali
...
2. Per provider personalizzati, controlla la config di HopCoderX e:
   - Assicurati che l'ID provider usato nel comando `/connect` corrisponda all'ID nella tua config HopCoderX.
