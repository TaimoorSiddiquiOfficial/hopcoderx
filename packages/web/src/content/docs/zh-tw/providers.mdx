...
HopCoderX Zen 是由 HopCoderX 團隊提供的模型列表，這些模型已經過測試和驗證，能夠與 HopCoderX 良好配合使用。[了解更多](/docs/zen)。

:::tip
如果您是新使用者，我們建議從 HopCoderX Zen 開始。
:::

1. 在 TUI 中執行 `/connect` 指令，選擇 HopCoderX，然後前往 [HopCoderX.ai/auth](https://HopCoderX.ai/auth)。
...
HopCoderX Zen 是由 HopCoderX 團隊提供的經過測試和驗證的模型列表。[了解更多](/docs/zen)。

1. 登入 **<a href={console}>HopCoderX Zen</a>** 並點擊 **Create API Key**。

2. 執行 `/connect` 指令並搜尋 **HopCoderX Zen**。
