...
HopCoderX Bdr 是由 HopCoderX 團隊提供的模型列表，這些模型已經過測試和驗證，能夠與 HopCoderX 良好配合使用。[了解更多](/docs/zen)。
...
:::note[合規說明]
HopCoderX 會使用一個小模型來執行部分 AI 任務，例如產生工作階段標題。預設情況下使用由 Zen 託管的 gpt-5-nano。如果您需要讓 HopCoderX 僅使用您自己的 GitLab 託管實例，請在 `HopCoderX.json` 檔案中新增以下內容。同時建議停用工作階段分享。
...
HopCoderX Bdr 是由 HopCoderX 團隊提供的經過測試和驗證的模型列表。[了解更多](/docs/zen)。
