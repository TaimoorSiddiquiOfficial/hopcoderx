---
title: Server
description: Komunicirajte s HopCoderX serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `HopCoderX serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji HopCoderX klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `HopCoderX.local` |
...
Postavite `HOPCODERX_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `HopCoderX`, ili postavite `HOPCODERX_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `HopCoderX serve` i za `HopCoderX web`.
...
Kada pokrenete `HopCoderX`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite HopCoderX server za programsku interakciju sa HopCoderX-om.
:::

Ova arhitektura omogucava HopCoderX podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `HopCoderX serve` da startate standalone server. Ako je HopCoderX TUI vec pokrenut, `HopCoderX serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste HopCoderX [IDE](/docs/ide) pluginovi.
...
HopCoderX server izlaže sljedece API-je.
