...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. HopCoderX dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u HopCoderX.
...
:::note
Ovaj alat je dostupan samo kada je `HOPCODERX_EXPERIMENTAL_LSP_TOOL=true` (ili `HOPCODERX_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz HopCoderX provajdera ili kada je varijabla `HOPCODERX_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju HopCoderX:
