---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u HopCoderX.
---

import config from "../../../../config.mjs"
export const console = config.console

HopCoderX koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj HopCoderX konfiguraciji.
...
u `~/.local/share/HopCoderX/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem HopCoderX
config.
...
HopCoderX Bdr je lista modela koje je obezbedio HopCoderX tim koji su bili
testirano i potvrđeno da dobro radi sa HopCoderX. [Saznajte više](/docs/bdr).

:::tip
Ako ste novi, preporučujemo da počnete sa HopCoderX Bdr.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite HopCoderX i idite na [hopcoderx.dev/auth](https://hopcoderx.dev/auth).
...
Radi kao i svaki drugi provajder u HopCoderX i potpuno je opcionalan za korištenje.
...
Da biste koristili Amazon Bedrock s HopCoderX:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete HopCoderX:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `HopCoderX.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u HopCoderX.
:::
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi HopCoderX ispravno radio.
   :::
...
Također možete dodati modele kroz svoju HopCoderX konfiguraciju.
...
da pohrani token u HopCoderX auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
HopCoderX koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Bdr. Da zaključate HopCoderX
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`HopCoderX.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [HopCoderX-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth).

##### Konfiguracija

Prilagodite putem `HopCoderX.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s HopCoderX:
...
Za korištenje Google Vertex AI s HopCoderX:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće HopCoderX.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem HopCoderXa, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [HopCoderX-helicone-session](https://github.com/H2Shami/HopCoderX-helicone-session) da automatski prijavite svaki HopCoderX razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki HopCoderX razgovor naveden kao zasebna sesija.
...
Možete konfigurirati HopCoderX za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati HopCoderX za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati HopCoderX za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za HopCoderX. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/HopCoderX) za detalje.
:::
...
Da biste koristili Ollama Cloud s HopCoderX:
...
3. Kopirajte API ključ za korištenje u HopCoderX.
...
6. **Važno**: Prije upotrebe modela oblaka u HopCoderX, morate lokalno povući informacije o modelu:
...
HopCoderX Bdr je lista testiranih i verifikovanih modela koju je obezbedio HopCoderX tim. [Saznajte više](/docs/bdr).

1. Prijavite se na **<a href={console}>HopCoderX Bdr</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **HopCoderX Bdr**.

```txt
   /connect
```

3. Unesite svoj HopCoderX API ključ.
...
Također možete dodati dodatne modele putem vaše HopCoderX konfiguracije.
...
5. Također ih možete prilagoditi putem vaše HopCoderX konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa HopCoderXom:
...
Također možete prilagoditi modele kroz svoju HopCoderX konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s HopCoderX-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `HopCoderX.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju HopCoderX da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `HopCoderX auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite HopCoderX konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj HopCoderX konfiguraciji.
