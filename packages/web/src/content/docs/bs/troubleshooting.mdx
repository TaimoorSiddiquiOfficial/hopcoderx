...
Da biste otklonili probleme s HopCoderX, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\HopCoderX\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `HopCoderX --log-level DEBUG`.

---

## Pohrana

HopCoderX pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/HopCoderX/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\HopCoderX`
...
HopCoderX Desktop pokreće lokalni HopCoderX server (`HopCoderX-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `HopCoderX` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/HopCoderX/HopCoderX.jsonc` (ili `~/.config/HopCoderX/HopCoderX.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/HopCoderX/HopCoderX.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\HopCoderX\HopCoderX.jsonc`
...
HopCoderX također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/HopCoderX/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\HopCoderX\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga HopCoderX mogao ponovo izgraditi.

1. Potpuno zatvorite HopCoderX Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/HopCoderX`
- **Linux**: obrišite `~/.cache/HopCoderX` (ili pokrenite `rm -rf ~/.cache/HopCoderX`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\HopCoderX`

3. Ponovo pokrenite HopCoderX Desktop.

---

### Rješavanje problema sa vezom na serveru

HopCoderX Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `HopCoderX.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `HOPCODERX_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `HOPCODERX_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, HopCoderX Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa HopCoderX karakteristikama.

---

### Obavještenja se ne prikazuju

HopCoderX Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za HopCoderX u postavkama vašeg OS-a, i
...
1. Zatvorite HopCoderX Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka HopCoderX Desktop aplikacije):

- `HopCoderX.settings.dat` (URL zadanog servera za desktop)
- `HopCoderX.global.dat` i `HopCoderX.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Ako imate problema s HopCoderX:
...
Da saznate kojim modelima imate pristup, pokrenite `HopCoderX models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\HopCoderX`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. HopCoderX dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\HopCoderX`

2. Ponovo pokrenite HopCoderX da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti HopCoderX da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
HopCoderX će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
