...
ove ili konfigurirajte svoje putem HopCoderX config.
...
HopCoderX dolazi sa dva ugrađena načina rada.
...
- `edit` - Ne mogu modificirati postojeće fajlove, osim fajlova koji se nalaze na `.HopCoderX/plans/*.md` radi detaljiziranja samog plana
...
Konfigurirajte načine rada u svom `HopCoderX.json` konfiguracijskom fajlu:
...
- Globalno: `~/.config/HopCoderX/modes/`
- Projekat: `.HopCoderX/modes/`
...
Ako temperatura nije navedena, HopCoderX koristi podrazumijevane postavke specifične za model (obično 0 za većinu modela i 0.55 za Qwen modele).
...
Kreirajte fajlove načina u `.HopCoderX/modes/` za specifične načine rada ili `~/.config/HopCoderX/modes/` za globalne načine:
