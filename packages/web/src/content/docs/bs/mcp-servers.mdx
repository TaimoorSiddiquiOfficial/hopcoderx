...
Možete dodati vanjske alate u HopCoderX koristeći _Model Context Protocol_, ili MCP. HopCoderX podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [HopCoderX Config](https://hopcoderx.dev/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/HopCoderX`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
HopCoderX automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, HopCoderX će:
...
Ako server zahtijeva autentifikaciju, HopCoderX će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `HopCoderX mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, HopCoderX sigurno čuva tokene u `~/.local/share/HopCoderX/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u HopCoderX, zajedno s ugrađenim alatima. Možete njima upravljati kroz HopCoderX konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete HopCoderX sa vašim Sentry nalogom.
