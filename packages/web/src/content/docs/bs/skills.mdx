...
Agent skills omogucavaju HopCoderX da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
HopCoderX pretrazuje ove lokacije:

- Konfiguracija projekta: `.HopCoderX/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/HopCoderX/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, HopCoderX ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.HopCoderX/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/HopCoderX/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.HopCoderX/skills/git-release/SKILL.md` ovako:
...
HopCoderX navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `HopCoderX.json`:
...
**Za ugradene agente** (u `HopCoderX.json`):
