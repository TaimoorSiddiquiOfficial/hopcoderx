...
Postoje dvije vrste agenata u HopCoderX: primarni agenti i podagenti.
...
HopCoderX dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
HopCoderX dolazi sa dva ugrađena subagenta, **General** i **Explore**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

HopCoderX dolazi sa dva ugrađena primarna agenta i dva ugrađena subagenta.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `HopCoderX.json`:
...
- Globalno: `~/.config/HopCoderX/agents/`
- Po projektu: `.HopCoderX/agents/`
...
Ako temperatura nije navedena, HopCoderX koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu HopCoderX konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj HopCoderX konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [HopCoderX Bdr](/docs/zen), koristili biste `HopCoderX/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `HopCoderX models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/TaimoorSiddiquiOfficial/hopcoderx).
:::
