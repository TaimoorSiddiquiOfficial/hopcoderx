...
U HopCoderX mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, HopCoderX koristi nasu `HopCoderX` temu.
...
HopCoderX dolazi sa vise ugradenih tema.
...
- Zele da HopCoderX odgovara izgledu njihovog terminala
...
HopCoderX podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/HopCoderX/themes/*.json` ili `$XDG_CONFIG_HOME/HopCoderX/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.HopCoderX/themes/*.json`
4. **Trenutni radni direktorij** - `./.HopCoderX/themes/*.json`
