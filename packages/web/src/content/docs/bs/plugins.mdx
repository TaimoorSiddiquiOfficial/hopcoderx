---
title: Dodaci
description: Napišite vlastite dodatke za proširenje HopCoderX.
---

Dodaci vam omogućavaju da proširite HopCoderX spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja HopCoderX.
...
- `.HopCoderX/plugins/` - Dodaci na nivou projekta
- `~/.config/HopCoderX/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/HopCoderX/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/HopCoderX/HopCoderX.json`)
2. Konfiguracija projekta (`HopCoderX.json`)
3. Globalni direktorij dodataka (`~/.config/HopCoderX/plugins/`)
4. Direktorij dodataka projekta (`.HopCoderX/plugins/`)
...
HopCoderX pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: HopCoderX SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje HopCoderX.
...
:::note
Ako koristite desktop aplikaciju HopCoderX, ona može automatski slati sistemske obavijesti kada je odgovor spreman ili kada dođe do greške u sesiji.
:::
...
Spriječite HopCoderX da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u HopCoderX:
...
Pomoćnik `tool` kreira prilagođeni alat koji HopCoderX može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za HopCoderX zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://hopcoderx.dev/docs/sdk) za detalje.
