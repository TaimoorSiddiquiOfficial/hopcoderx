...
For at fejlfinde problemer med HopCoderX, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\HopCoderX\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `HopCoderX --log-level DEBUG`.

---

## Lagring

HopCoderX gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/HopCoderX/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\HopCoderX`
...
HopCoderX Desktop kører en lokal HopCoderX-server (`HopCoderX-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `HopCoderX`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/HopCoderX/HopCoderX.jsonc` (eller `~/.config/HopCoderX/HopCoderX.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/HopCoderX/HopCoderX.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\HopCoderX\HopCoderX.jsonc`
...
HopCoderX kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/HopCoderX/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\HopCoderX\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så HopCoderX kan genopbygge den.

1. Afslut HopCoderX Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/HopCoderX`
- **Linux**: slet `~/.cache/HopCoderX` (eller kør `rm -rf ~/.cache/HopCoderX`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\HopCoderX`

3. Start HopCoderX Desktop på ny.

---

### Løs problemer med serverforbindelse

HopCoderX Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `HopCoderX.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `HOPCODERX_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `HOPCODERX_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver HopCoderX Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i HopCoderX.

---

### Meddelelser vises ikke

HopCoderX Desktop viser kun systemvarsler når:

- varsler er aktiveret for HopCoderX i dine OS-indstillinger, og
...
1. Afslut HopCoderX Desktop.
2. Find og slet disse filer (de findes i HopCoderX Desktop-appens datamappe):

- `HopCoderX.settings.dat` (skrivebordsstandardserver URL)
- `HopCoderX.global.dat` og `HopCoderX.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med HopCoderX:
...
For at finde ud af hvilke modeller du har adgang til, kør `HopCoderX models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\HopCoderX`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. HopCoderX installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\HopCoderX`

2. Start HopCoderX på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge HopCoderX til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
HopCoderX vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
