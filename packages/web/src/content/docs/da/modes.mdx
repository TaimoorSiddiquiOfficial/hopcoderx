...
:::caution
Tilstande er nu konfigureret gennem indstillingen `agent` i HopCoderX-konfigurationen. De
`mode` mulighed er nu forældet. [Learn more](/docs/agents).
:::

Tilstande i HopCoderX giver dig mulighed for at tilpasse adfærd, værktøjer og prompter til forskellige brugstilfælde.

Den kommer med to indbyggede tilstande: **build** og **plan**. Du kan tilpasse
disse eller konfigurer dine egne gennem HopCoderX-konfigurationen.
...
HopCoderX leveres med to indbyggede tilstande.
...
- `edit` - Kan ikke ændre eksisterende filer, undtagen filer placeret på `.HopCoderX/plans/*.md` for at detaljere selve planen
...
Konfigurer tilstande i din `HopCoderX.json`-konfigurationsfil:
...
- Globalt: `~/.config/HopCoderX/modes/`
...
Hvis der ikke er angivet nogen temperatur, bruger HopCoderX modelspecifikke standarder (typisk 0 for de fleste modeller, 0,55 for Qwen-modeller).
...
både den globale HopCoderX-konfiguration og den projektspecifikke konfiguration.
...
Opret tilstandsfiler i `.HopCoderX/modes/` for projektspecifikke tilstande eller `~/.config/HopCoderX/modes/` for globale tilstande:
