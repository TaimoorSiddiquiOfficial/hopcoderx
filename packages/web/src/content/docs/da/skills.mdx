...
Agentferdigheter lar HopCoderX oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
HopCoderX søker etter disse stedene:

- Prosjektkonfigurasjon: `.HopCoderX/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/HopCoderX/skills/<name>/SKILL.md`
...
For prosjektlokale stier går HopCoderX opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.HopCoderX/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/HopCoderX/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.HopCoderX/skills/git-release/SKILL.md` slik:
...
HopCoderX viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `HopCoderX.json`:
...
**For innebygde agenter** (i `HopCoderX.json`):
