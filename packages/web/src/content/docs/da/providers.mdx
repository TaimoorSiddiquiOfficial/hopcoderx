---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i HopCoderX.
---

import config from "../../../../config.mjs"
export const console = config.console

HopCoderX bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i HopCoderX-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/HopCoderX/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i HopCoderX-konfigurationen.
...
HopCoderX Bdr er en liste over modeller leveret af HopCoderX-teamet, som er blevet testet og verificeret til at fungere godt med HopCoderX. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med HopCoderX Bdr.
:::

1. Kør kommandoen `/connect` i TUI, vælg HopCoderX og gå til [hopcoderx.dev/auth](https://hopcoderx.dev/auth).
...
Det fungerer som alle andre udbydere i HopCoderX og er helt valgfrit at bruge.
...
Sådan bruger du Amazon Bedrock med HopCoderX:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører HopCoderX:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `HopCoderX.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i HopCoderX understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at HopCoderX skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem HopCoderX-konfigurationen.
...
gemme tokenet i HopCoderX auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
HopCoderX bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse HopCoderX
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`HopCoderX.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [HopCoderX-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `HopCoderX.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med HopCoderX:
...
Sådan bruger du Google Vertex AI med HopCoderX:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører HopCoderX.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem HopCoderX, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [HopCoderX-helicone-session](https://github.com/H2Shami/HopCoderX-helicone-session) plugin for automatisk at logge hver HopCoderX-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver HopCoderX-samtale opført som en separat session.
...
Du kan konfigurere HopCoderX til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere HopCoderX til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere HopCoderX til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for HopCoderX. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/HopCoderX) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med HopCoderX:
...
3. Kopier API-nøglen til brug i HopCoderX.
...
6. **Vigtigt**: Før du bruger skymodeller i HopCoderX, skal du hente modelinformationen lokalt:
...
HopCoderX Bdr er en liste over testede og verificerede modeller leveret af HopCoderX-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>HopCoderX Bdr</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **HopCoderX Bdr**.

   ```txt
   /connect
   ```

3. Indtast HopCoderX API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem HopCoderX-konfigurationen.
...
5. Du kan også tilpasse dem gennem HopCoderX-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med HopCoderX:
...
Du kan også tilpasse modeller gennem HopCoderX-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med HopCoderX. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `HopCoderX.json`-filen i projektkataloget:
...
`limit`-felterne lader HopCoderX forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `HopCoderX auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek HopCoderX-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i HopCoderX-konfigurationen.
