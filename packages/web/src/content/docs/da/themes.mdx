...
Med HopCoderX kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger HopCoderX vårt eget `HopCoderX`-tema.
...
HopCoderX kommer med flere innebygde temaer.
...
- Vil at HopCoderX skal matche terminalens utseende
...
HopCoderX støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/HopCoderX/themes/*.json` eller `$XDG_CONFIG_HOME/HopCoderX/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.HopCoderX/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.HopCoderX/themes/*.json`
