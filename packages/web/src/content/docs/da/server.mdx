---
title: Server
description: Interager med HopCoderX-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `HopCoderX serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en HopCoderX-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `HopCoderX.local` |
...
Indstil `HOPCODERX_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `HopCoderX`, eller indstil `HOPCODERX_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `HopCoderX serve` og `HopCoderX web`.
...
Når du kører `HopCoderX` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug HopCoderX-serveren til at interagere med HopCoderX programmatisk.
:::

Denne arkitektur lader HopCoderX understøtte flere klienter og lader dig interagere med HopCoderX programmatisk.

Du kan køre `HopCoderX serve` for at starte en selvstændig server. Hvis du har
HopCoderX TUI kørende, vil `HopCoderX serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af HopCoderX [IDE](/docs/ide) plugins.
...
HopCoderX-serveren viser følgende API'er.
