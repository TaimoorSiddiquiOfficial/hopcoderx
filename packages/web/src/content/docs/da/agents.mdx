...
Der er to typer agenter i HopCoderX; primære agenter og subagenter.
...
HopCoderX leveres med indbyggede primære agenter, **Build** og **Plan**. Vi vil
...
HopCoderX leveres med to indbyggede underagenter, **Generelt** og **Udforsk**. Vi vil se på dette nedenfor.

---

## Indbyggede

HopCoderX leveres med to indbyggede primære agenter og to indbyggede subagenter.
...
Konfigurer agent i din `HopCoderX.json`-konfigurationsfil:
...
Hvis ingen temperatur er angivet, bruger HopCoderX modelspecifikke standardindstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne sti er i forhold til, hvor konfigurationsfilen er placeret. Så dette virker både for den globale HopCoderX-konfiguration og den projektspecifikke konfiguration.
...
Model-id'et i din HopCoderX-konfiguration brugerformatet `provider/model-id`. For eksempel, hvis du bruger [HopCoderX Bdr](/docs/zen), vil du bruge `HopCoderX/gpt-5.1-codex` til GPT 5.1 Codex.
...
:::tip
Kør `HopCoderX models` for at se en liste over de tilgængelige modeller.
:::
...
:::tip
Har du en agent, du gerne vil dele? [Send en PR](https://github.com/TaimoorSiddiquiOfficial/hopcoderx).
:::
