...
Her er alle de innebygde verktøyene som er tilgængelige i HopCoderX.
...
:::note
Dette verktøyet er bare tilgængelig når `HOPCODERX_EXPERIMENTAL_LSP_TOOL=true` (eller `HOPCODERX_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger HopCoderX-leverandøren eller når miljøvariabelen `HOPCODERX_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
