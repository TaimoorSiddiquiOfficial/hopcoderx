---
title: Konfiguration
description: Ved at bruge HopCoderX JSON konfig.
---

Du kan konfigurere HopCoderX ved hjælp af en JSON-konfigurationsfil.

---

## Format

HopCoderX understøtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
For eksempel, hvis dine globale konfigurationssæt `theme: "hopcoderx"` og `autoupdate: true`, og dine projektkonfigurationssæt `model: "anthropic/claude-sonnet-4-5"`, vil den endelige konfiguration integrere alle tre indstillinger.
...
1. **Fjernkonfiguration** (fra `.well-known/HopCoderX`) - organisatoriske standardindstillinger
2. **Global config** (`~/.config/HopCoderX/HopCoderX.json`) - brugerpræferencer
3. **Tilpasset konfiguration** (`HOPCODERX_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`HopCoderX.json` i projekt) - projektspecifikke indstillinger
5. **`.HopCoderX` mapper** - agent, kommandoer, plugins
6. **Inline config** (`HOPCODERX_CONFIG_CONTENT` env var) - runtime tilsidesættelser
...
:::note
`.HopCoderX` og `~/.config/HopCoderX` mapperne bruger **flertalsnavne** for undermapper: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkelte navne (f.eks. `agent/`) understøtter også bagudkompatibilitet.
:::
...
Organisationer kan levere standardkonfiguration via `.well-known/HopCoderX`-slutpunktet. Dette hentes automatisk, når du godkender med en udbyder, der understøtter det.
...
Placer din globale HopCoderX-konfiguration i `~/.config/HopCoderX/HopCoderX.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Tilføj `HopCoderX.json` i dit projektrod. Project config har den højeste forrang blandt standard config-filer - den tilsidesætter både globale og eksterne config.

:::tip
Placer projektspecifik konfiguration i roden af ​​dit projekt.
:::

Når HopCoderX starter op, søger den efter en konfigurationsfil i den aktuelle mappe eller går op til den nærmeste Git-mappe.
...
Angiv en brugerdefineret konfigurationsfilsti ved hjælp af miljøvariablen `HOPCODERX_CONFIG`.
...
Angiv en brugerdefineret konfigurationsmappe ved hjælp af `HOPCODERX_CONFIG_DIR`
miljøvariabel. Dette kort vil blive søgt efter agenter, kommandoer,
modes og plugins ligesom standard `.HopCoderX` mappen, og bør
...
Den brugerdefinerede mappe indlæses efter den globale konfig og `.HopCoderX` mapper, så den **kan tilsidesætte** deres indstillinger.

---

## Skema

Konfigurationsfilen har et skema, der defineres i [**`hopcoderx.dev/config.json`**](https://hopcoderx.dev/config.json).
...
Du kan konfigurere serverindstillinger for kommandoerne `HopCoderX serve` og `HopCoderX web` gennem indstillingen `server`.
...
- `mdns` - Aktiver mDNS-tjenesteopdagelse. Dette gør det muligt for andre enheder på netværket at opdage din HopCoderX-server.
- `mdnsDomain` - Brugerdefineret domænenavn til mDNS-tjeneste. Som standard er `HopCoderX.local`. Nyttigt til at køre flere forekomster på det samme netværk.
...
Du kan konfigurere de udbydere og modeller, du vil bruge i din HopCoderX-konfiguration, gennem mulighederne `provider`, `model` og `small_model`.
...
Indstillingen `small_model` konfigurerer en separat model til lette opgaver som titelgenerering. Som standard forsøger HopCoderX at bruge en billigere model, hvis en er tilgængelig fra din udbyder, ellers falder den tilbage til din hovedmodel.
...
Du kan konfigurere det tema, du vil bruge i din HopCoderX-konfiguration, gennem indstillingen `theme`.
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/HopCoderX/agents/` eller `.HopCoderX/agents/`. [Learn more here](/docs/agents).
...
Standardagenten skal være en primær agent (ikke en subagent). Dette kan være en indbygget agent som `"build"` eller `"plan"`, eller en [custom agent](/docs/agents), du har defineret. Hvis den angivne agent ikke eksisterer eller er en underagent, vil HopCoderX falde tilbage til `"build"` med en advarsel.

Denne indstilling gælder på tværs af alle grænseflader: TUI, CLI (`HopCoderX run`), desktop-app og GitHub Action.
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/HopCoderX/commands/` eller `.HopCoderX/commands/`. [Learn more here](/docs/commands).
...
HopCoderX vil automatisk downloade alle nye opdateringer, når den starter op. Du kan deaktivere dette med indstillingen `autoupdate`.
...
Som standard HopCoderX **tillader alle operationer** uden at kræve eksplicit godkendelse. Du kan ændre dette ved at bruge indstillingen `permission`.
...
[Plugins](/docs/plugins) udvide HopCoderX med brugerdefinerede værktøjer, kroge og integrationer.

Placer plugin-filer i `.HopCoderX/plugins/` eller `~/.config/HopCoderX/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.
...
Dette er nyttigt, når du vil begrænse HopCoderX til kun at bruge specifikke udbydere i stedet for at deaktivere dem én efter én.
