---
title: Plugins
description: Skriv dine egne plugins for at udvide HopCoderX.
---

Plugins giver dig mulighed for at udvide HopCoderX ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre HopCoderXs standardadfærd.
...
- `.HopCoderX/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/HopCoderX/node_modules/`.
...
1. Global konfiguration (`~/.config/HopCoderX/HopCoderX.json`)
2. Projektkonfiguration (`HopCoderX.json`)
3. Global plugin-mappe (`~/.config/HopCoderX/plugins/`)
4. Projekt plugin bibliotek (`.HopCoderX/plugins/`)
...
HopCoderX kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En HopCoderX SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide HopCoderX.
...
:::note
Hvis du bruger HopCoderX desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå HopCoderX i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til HopCoderX:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som HopCoderX kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for HopCoderX sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://hopcoderx.dev/docs/sdk) for detaljer.
