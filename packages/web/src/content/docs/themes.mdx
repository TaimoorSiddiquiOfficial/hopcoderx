...
With HopCoderX you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, HopCoderX uses our own `HopCoderX` theme.
...
HopCoderX comes with several built-in themes.
...
HopCoderX supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/HopCoderX/themes/*.json` or `$XDG_CONFIG_HOME/HopCoderX/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.HopCoderX/themes/*.json`
4. **Current working directory** - Defined in `./.HopCoderX/themes/*.json`
