...
:::caution
يتم الآن ضبط الأوضاع عبر خيار `agent` في إعدادات HopCoderX. أصبح خيار
`mode` مُهمَلًا الآن. [اعرف المزيد](/docs/agents).
:::

تتيح لك الأوضاع في HopCoderX تخصيص السلوك والأدوات والمطالبات لحالات استخدام مختلفة.

يأتي مع وضعين مدمجين: **build** و **plan**. يمكنك تخصيصهما أو إعداد
أوضاعك الخاصة عبر إعدادات HopCoderX.
...
يأتي HopCoderX مع وضعين مدمجين.
...
- `edit` - لا يمكن تعديل الملفات الموجودة، باستثناء الملفات الموجودة في `.HopCoderX/plans/*.md` لتفصيل الخطة نفسها
...
قم بإعداد الأوضاع في ملف الإعدادات `HopCoderX.json` لديك:
...
- عالمي: `~/.config/HopCoderX/modes/`
- للمشروع: `.HopCoderX/modes/`
...
إذا لم يتم تحديد درجة الحرارة، يستخدم HopCoderX القيم الافتراضية الخاصة بكل نموذج (عادةً 0 لمعظم النماذج و 0.55 لنماذج Qwen).
...
إعدادات HopCoderX العالمية وكذلك إعدادات المشروع.
...
أنشئ ملفات الأوضاع في `.HopCoderX/modes/` لأوضاع خاصة بالمشروع أو في `~/.config/HopCoderX/modes/` لأوضاع عالمية:
