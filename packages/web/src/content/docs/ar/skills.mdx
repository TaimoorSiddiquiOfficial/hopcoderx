...
تتيح مهارات الوكيل لـ HopCoderX اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث HopCoderX في هذه المواقع:

- إعدادات المشروع: `.HopCoderX/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/HopCoderX/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج HopCoderX صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.HopCoderX/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/HopCoderX/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.HopCoderX/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض HopCoderX المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `HopCoderX.json`:
...
**للوكلاء المدمجين** (في `HopCoderX.json`):
