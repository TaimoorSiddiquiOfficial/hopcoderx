...
أنشئ الملف `.HopCoderX/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات HopCoderX أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ HopCoderX:
...
- على مستوى النظام: `~/.config/HopCoderX/commands/`
- لكل مشروع: `.HopCoderX/commands/`
...
يتضمن HopCoderX عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
