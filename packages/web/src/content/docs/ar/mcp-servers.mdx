...
يمكنك إضافة أدوات خارجية إلى HopCoderX باستخدام _Model Context Protocol_، أو MCP. يدعم HopCoderX خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات HopCoderX](https://hopcoderx.dev/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/HopCoderX` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى HopCoderX تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم HopCoderX بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك HopCoderX المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `HopCoderX mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن HopCoderX الرموز بشكل آمن في `~/.local/share/HopCoderX/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل HopCoderX إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات HopCoderX مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط HopCoderX بحساب Sentry الخاص بك.
