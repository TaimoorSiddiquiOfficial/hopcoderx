...
مع HopCoderX يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم HopCoderX سمتنا `HopCoderX`.
...
يأتي HopCoderX مع عدة سمات مدمجة.
...
- يريدون أن يطابق HopCoderX مظهر terminal لديهم
...
يدعم HopCoderX نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/HopCoderX/themes/*.json` أو `$XDG_CONFIG_HOME/HopCoderX/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.HopCoderX/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.HopCoderX/themes/*.json`
