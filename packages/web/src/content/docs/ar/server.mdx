---
title: الخادم
description: تفاعل مع خادم HopCoderX عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `HopCoderX serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل HopCoderX استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `HopCoderX.local` |
...
عيّن `HOPCODERX_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `HopCoderX`، أو عيّن `HOPCODERX_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `HopCoderX serve` و `HopCoderX web`.
...
عند تشغيل `HopCoderX` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم HopCoderX للتفاعل مع HopCoderX برمجيا.
:::

تتيح هذه البنية لـ HopCoderX دعم عدة عملاء وتمكّنك من التفاعل مع HopCoderX برمجيا.

يمكنك تشغيل `HopCoderX serve` لبدء خادم مستقل. إذا كانت واجهة HopCoderX في terminal (TUI)
قيد التشغيل، فسيبدأ `HopCoderX serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات HopCoderX لـ [IDE](/docs/ide).
...
يعرض خادم HopCoderX واجهات API التالية.
