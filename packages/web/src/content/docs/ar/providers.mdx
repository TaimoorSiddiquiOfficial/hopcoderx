---
title: المزوّدون
description: استخدام أي موفّر LLM في HopCoderX.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم HopCoderX ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات HopCoderX.
...
في `~/.local/share/HopCoderX/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات HopCoderX.
...
HopCoderX Bdr هي قائمة نماذج يوفّرها فريق HopCoderX وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع HopCoderX. [اعرف المزيد](/docs/bdr).

:::tip
إذا كنت جديدا، فننصح بالبدء مع HopCoderX Bdr.
:::

1. شغّل الأمر `/connect` في TUI، واختر HopCoderX، ثم انتقل إلى [hopcoderx.dev/auth](https://hopcoderx.dev/auth).
...
يعمل مثل أي مزوّد آخر في HopCoderX واستخدامه اختياري بالكامل.
...
لاستخدام Amazon Bedrock مع HopCoderX:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل HopCoderX:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `HopCoderX.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل HopCoderX غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل HopCoderX بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات HopCoderX.
...
تخزين الرمز في مساحة تخزين مصادقة HopCoderX.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم HopCoderX نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Bdr. لحصر HopCoderX
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`HopCoderX.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [HopCoderX-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth).

##### التهيئة

خصّص عبر `HopCoderX.json`:
...
لاستخدام اشتراك GitHub Copilot مع HopCoderX:
...
لاستخدام Google Vertex AI مع HopCoderX:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر HopCoderX، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [HopCoderX-helicone-session](https://github.com/H2Shami/HopCoderX-helicone-session) لتسجيل كل محادثة HopCoderX تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة HopCoderX مدرجة كجلسة مستقلة.
...
يمكنك ضبط HopCoderX لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط HopCoderX لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط HopCoderX لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع HopCoderX. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/HopCoderX) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع HopCoderX:
...
3. انسخ مفتاح API لاستخدامه في HopCoderX.
...
6. **مهم**: قبل استخدام نماذج السحابة في HopCoderX، يجب سحب معلومات النموذج محليا:
...
HopCoderX Bdr هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق HopCoderX. [اعرف المزيد](/docs/bdr).

1. سجّل الدخول إلى **<a href={console}>HopCoderX Bdr</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **HopCoderX Bdr**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ HopCoderX.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات HopCoderX.
...
5. يمكنك أيضا تخصيصها عبر إعدادات HopCoderX. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع HopCoderX:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات HopCoderX. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع HopCoderX. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `HopCoderX.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ HopCoderX بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `HopCoderX auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات HopCoderX وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات HopCoderX.
