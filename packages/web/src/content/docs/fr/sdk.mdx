---
title: SDK
description: Client JS de type sécurisé pour le serveur HopCoderX.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK HopCoderX JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler HopCoderX par programme.
...
Créez une instance de HopCoderX :

```javascript
import { createHopCoderX } from "@HopCoderX-ai/sdk"

const { client } = await createHopCoderX()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `HopCoderX.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${HopCoderX.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de HopCoderX, vous pouvez créer une instance client pour vous y connecter :

```javascript
import { createHopCoderXClient } from "@HopCoderX-ai/sdk"
...
```typescript
import type { Session, Message, Part } from "@HopCoderX-ai/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*HopCoderX" },
})
