...
Il existe deux types d'agents dans HopCoderX ; agents primaires et sous-agents.
...
HopCoderX est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
HopCoderX est livré avec deux sous-agents intégrés, **General** et **Explore**. Nous verrons cela ci-dessous.

---

## Agents intégrés

HopCoderX est livré avec deux agents primaires intégrés et deux sous-agents intégrés.
...
Configurez les agents dans votre fichier de configuration `HopCoderX.json` :
...
- Global : `~/.config/HopCoderX/agents/`
- Par projet : `.HopCoderX/agents/`

```markdown title="~/.config/HopCoderX/agents/review.md"
---
...
Si aucune température n'est spécifiée, HopCoderX utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale HopCoderX et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration HopCoderX utilise le format `provider/model-id`. Par exemple, si vous utilisez [HopCoderX Bdr](/docs/zen), vous utiliserez `HopCoderX/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```json title="HopCoderX.json" {3-6,9-12}
{
  "$schema": "https://hopcoderx.dev/config.json",
  "tools": {
    "write": true,
...
```json title="HopCoderX.json" {3-5,8-10}
{
  "$schema": "https://hopcoderx.dev/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Exécutez `HopCoderX models` pour voir une liste des modèles disponibles.
:::
...
:::tip
Avez-vous un agent que vous aimeriez partager ? [Soumettez un PR](https://github.com/TaimoorSiddiquiOfficial/hopcoderx).
:::
...
```markdown title="~/.config/HopCoderX/agents/docs-writer.md"
---
...
```markdown title="~/.config/HopCoderX/agents/security-auditor.md"
---
