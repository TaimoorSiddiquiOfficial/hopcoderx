---
title: Serveur
description: Interagissez avec le serveur HopCoderX via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `HopCoderX serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client HopCoderX peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `HopCoderX.local` |
...
Définissez `HOPCODERX_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `HopCoderX`, ou définissez `HOPCODERX_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `HopCoderX serve` et à `HopCoderX web`.
...
Lorsque vous exécutez `HopCoderX`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur HopCoderX pour interagir avec HopCoderX par programmation.
:::

Cette architecture permet à HopCoderX de prendre en charge plusieurs clients et vous permet d'interagir avec HopCoderX par programme.

Vous pouvez exécuter `HopCoderX serve` pour démarrer un serveur autonome. Si vous avez le
HopCoderX TUI en cours d'exécution, `HopCoderX serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins HopCoderX [IDE](/docs/ide).
...
Le serveur HopCoderX expose les API suivantes.
