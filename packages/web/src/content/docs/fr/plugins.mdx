---
title: Plugins
description: Écrivez vos propres plugins pour étendre HopCoderX.
---

Les plugins vous permettent d'étendre HopCoderX en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de HopCoderX.
...
- `.HopCoderX/plugins/` - Plugins au niveau du projet
- `~/.config/HopCoderX/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/HopCoderX/node_modules/`.
...
1. Configuration globale (`~/.config/HopCoderX/HopCoderX.json`)
2. Configuration du projet (`HopCoderX.json`)
3. Répertoire global des plugins (`~/.config/HopCoderX/plugins/`)
4. Répertoire des plugins du projet (`.HopCoderX/plugins/`)
...
HopCoderX exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK HopCoderX pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre HopCoderX.
...
:::note
Si vous utilisez l'application de bureau HopCoderX, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez HopCoderX de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à HopCoderX :
...
L'assistant `tool` crée un outil personnalisé que HopCoderX peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour HopCoderX aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://hopcoderx.dev/docs/sdk) pour plus de détails.
