...
Avec HopCoderX, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, HopCoderX utilise notre propre thème `HopCoderX`.
...
HopCoderX est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que HopCoderX corresponde à l'apparence de leur terminal
...
HopCoderX prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/HopCoderX/themes/*.json` ou `$XDG_CONFIG_HOME/HopCoderX/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.HopCoderX/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.HopCoderX/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/HopCoderX/themes
vim ~/.config/HopCoderX/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .HopCoderX/themes
vim .HopCoderX/themes/my-theme.json
```
...
{
  "$schema": "https://hopcoderx.dev/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
