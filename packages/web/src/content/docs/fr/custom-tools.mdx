...
description: Créez des outils que le LLM peut appeler dans HopCoderX.
---

Les outils personnalisés sont des fonctions que vous créez et que le LLM peut appeler pendant les conversations. Ils fonctionnent avec les [outils intégrés](/docs/tools) de HopCoderX comme `read`, `write` et `bash`.
...
- Localement en les plaçant dans le répertoire `.HopCoderX/tools/` de votre projet.
- Ou globalement, en les plaçant dans `~/.config/HopCoderX/tools/`.
...
```ts title=".HopCoderX/tools/database.ts" {1}
import { tool } from "@HopCoderX-ai/plugin"
...
```ts title=".HopCoderX/tools/math.ts"
import { tool } from "@HopCoderX-ai/plugin"
...
```ts title=".HopCoderX/tools/project.ts" {8}
import { tool } from "@HopCoderX-ai/plugin"
...
```python title=".HopCoderX/tools/add.py"
import sys
...
```ts title=".HopCoderX/tools/python-add.ts" {10}
import { tool } from "@HopCoderX-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".HopCoderX/tools/add.py")
