---
title: Custom Tools
description: Create tools the LLM can call in HopCoderX.
---

Custom tools are functions you create that the LLM can call during conversations. They work alongside HopCoderX's [built-in tools](/docs/tools) like `read`, `write`, and `bash`.
...
- Locally by placing them in the `.HopCoderX/tools/` directory of your project.
- Or globally, by placing them in `~/.config/HopCoderX/tools/`.
...
```ts title=".HopCoderX/tools/bash.ts"
import { tool } from "@HopCoderX-ai/plugin"
