---
title: Server
description: Interact with HopCoderX server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `HopCoderX serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an HopCoderX client can use.
...
Set `HOPCODERX_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `HopCoderX`, or set `HOPCODERX_SERVER_USERNAME` to override it. This applies to both `HopCoderX serve` and `HopCoderX web`.
...
When you run `HopCoderX` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the HopCoderX server to interact with HopCoderX programmatically.
:::

This architecture lets HopCoderX support multiple clients and allows you to interact with HopCoderX programmatically.

You can run `HopCoderX serve` to start a standalone server. If you have the
HopCoderX TUI running, `HopCoderX serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the HopCoderX [IDE](/docs/ide) plugins.
...
The HopCoderX server exposes the following APIs.
