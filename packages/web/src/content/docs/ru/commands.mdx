...
Создайте `.HopCoderX/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию HopCoderX или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем HopCoderX [config](/docs/config):
...
- Глобальный: `~/.config/HopCoderX/commands/`
- Для каждого проекта: `.HopCoderX/commands/`
...
HopCoderX включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
