...
description: HopCoderX, LSP sunucularınızla bütünleşir.
---

HopCoderX, LLM'in kod tabanınızla etkileşime girmesine yardımcı olmak için Dil Sunucusu Protokolünüzle (LSP) bütünleşir. LLM'ye geri bildirim sağlamak için tanılamayı kullanır.

---

## Yerleşik

HopCoderX, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`HOPCODERX_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
HopCoderX bir dosyayı açtığında:
...
LSP sunucularını, HopCoderX yapılandırmanızdaki `lsp` bölümü aracılığıyla özelleştirebilirsiniz.
