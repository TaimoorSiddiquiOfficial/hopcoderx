...
Ajan becerileri, HopCoderX'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
HopCoderX su konumlari tarar:

- Proje konfigurasyonu: `.HopCoderX/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/HopCoderX/skills/<name>/SKILL.md`
...
Proje yerel yollarinda HopCoderX, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.HopCoderX/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/HopCoderX/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.HopCoderX/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
HopCoderX, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`HopCoderX.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`HopCoderX.json` icinde):
