...
HopCoderX ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak HopCoderX kendi `HopCoderX` temasını kullanır.
...
HopCoderX birden fazla yerleşik temayla gelir.
...
- HopCoderX'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
HopCoderX, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/HopCoderX/themes/*.json` veya `$XDG_CONFIG_HOME/HopCoderX/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.HopCoderX/themes/*.json`
4. **Geçerli çalışma dizini** - `./.HopCoderX/themes/*.json`
