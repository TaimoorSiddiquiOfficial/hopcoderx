---
title: Eklentiler
description: HopCoderX'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek HopCoderX'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya HopCoderX'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.HopCoderX/plugins/` - Proje düzeyinde eklentiler
- `~/.config/HopCoderX/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/HopCoderX/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/HopCoderX/HopCoderX.json`)
2. Project config (`HopCoderX.json`)
3. Global eklenti dizini (`~/.config/HopCoderX/plugins/`)
4. Proje eklenti dizini (`.HopCoderX/plugins/`)
...
HopCoderX bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşime geçmek için HopCoderX'lu bir SDK istemcisi.
...
HopCoderX'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
HopCoderX masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
HopCoderX'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca HopCoderX'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, HopCoderX'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK documentation](https://hopcoderx.dev/docs/sdk)'e bakın.
