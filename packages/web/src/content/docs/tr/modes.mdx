...
:::caution
Modlar artık HopCoderX yapılandırmasındaki `agent` seçeneği aracılığıyla yapılandırılıyor.
`mode` seçeneği artık kullanımdan kaldırıldı. [Daha fazla bilgi](/docs/agents).
:::

HopCoderX'daki modlar, farklı kullanım durumları için davranışı, araçları ve istemleri özelleştirmenize olanak tanır.

İki yerleşik modla birlikte gelir: **build** ve **plan**. Kişiselleştirebilirsiniz
bunları veya HopCoderX yapılandırması aracılığıyla kendinizinkini yapılandırın.
...
HopCoderX iki yerleşik modla birlikte gelir.
...
- `edit` - Planın kendisini detaylandırmak için `.HopCoderX/plans/*.md` adresinde bulunan dosyalar dışında mevcut dosyalar değiştirilemez
...
`HopCoderX.json` yapılandırma dosyanızdaki modları yapılandırın:
...
Sıcaklık belirtilmezse, HopCoderX modele özgü varsayılanları kullanır (çoğu model için genellikle 0, Qwen modelleri için 0,55).
...
hem global HopCoderX yapılandırması hem de projeye özel yapılandırma.
...
Projeye özel modlar için `.HopCoderX/modes/` veya genel modlar için `~/.config/HopCoderX/modes/`'de mod dosyaları oluşturun:
