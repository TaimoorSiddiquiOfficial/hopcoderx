---
title: Sunucu
description: HopCoderX sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`HopCoderX serve` komutu, HopCoderX istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `HopCoderX.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `HOPCODERX_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `HopCoderX` değeridir; değiştirmek isterseniz `HOPCODERX_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `HopCoderX serve` hem de `HopCoderX web` için geçerlidir.
...
`HopCoderX` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
HopCoderX ile programatik etkileşim için HopCoderX sunucusunu kullanın.
:::

Bu mimari, HopCoderX'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`HopCoderX serve` ile bağımsız bir sunucu başlatabilirsiniz. HopCoderX TUI açıksa, `HopCoderX serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum HopCoderX [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
HopCoderX sunucusu aşağıdaki API'leri sunar.
