---
title: Sağlayıcılar
description: HopCoderX'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

HopCoderX, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı HopCoderX yapılandırmanızda yapılandırın.
...
`~/.local/share/HopCoderX/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları HopCoderX'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
HopCoderX Bdr, HopCoderX ekibi tarafından sağlanan modellerin bir listesidir.
HopCoderX ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz HopCoderX Bdr ile başlamanızı öneririz.
:::
...
HopCoderX'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.
...
Amazon Bedrock'u HopCoderX ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   HopCoderX'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `HopCoderX.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi HopCoderX'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   HopCoderX'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   HopCoderX yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
to store token in HopCoderX auth storage.
:::

##### Kendi Kendine Barındırılan GitLab

:::note[uyumluluk notu]
HopCoderX, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. HopCoderX'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`HopCoderX.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [HopCoderX-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth) ana sayfasında.

##### Yapılandırma

`HopCoderX.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi HopCoderX'la kullanmak için:
...
Google Vertex AI'yi HopCoderX ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları HopCoderX çalıştırılırken ayarlayın.
...
Helicone'un HopCoderX aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her HopCoderX görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [HopCoderX-helicone-session](https://github.com/H2Shami/HopCoderX-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her HopCoderX görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
HopCoderX'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla HopCoderX'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla HopCoderX'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini HopCoderX için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/HopCoderX)'a bakın.
:::
...
Ollama Cloud'u HopCoderX ile kullanmak için:
...
3. HopCoderX'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: HopCoderX'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
HopCoderX Bdr, HopCoderX ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>HopCoderX Bdr</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **HopCoderX Bdr**'i arayın.

   ```txt
   /connect
   ```

3. HopCoderX API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   HopCoderX yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca HopCoderX yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi HopCoderX ile kullanmak için:
...
Modelleri HopCoderX yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı HopCoderX ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `HopCoderX.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları HopCoderX'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `HopCoderX auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için HopCoderX yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, HopCoderX yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
