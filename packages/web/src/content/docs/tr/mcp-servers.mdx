...
_Model Bağlam Protokolü_ veya MCP kullanarak HopCoderX'a harici araçlar ekleyebilirsiniz. HopCoderX hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [HopCoderX Config](https://hopcoderx.dev/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/HopCoderX` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
HopCoderX, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde HopCoderX şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa HopCoderX, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `HopCoderX mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından HopCoderX, belirteçleri `~/.local/share/HopCoderX/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra HopCoderX'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi HopCoderX yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve HopCoderX'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
