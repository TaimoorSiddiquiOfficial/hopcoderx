...
HopCoderX'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
HopCoderX, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
HopCoderX, **General** ve **Explore** olmak üzere iki yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

HopCoderX iki yerleşik birincil agent ve iki yerleşik alt agent ile birlikte gelir.
...
Agent'ları `HopCoderX.json` yapılandırma dosyanızda yapılandırın:
...
Sıcaklık belirtilmezse HopCoderX modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global HopCoderX yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
HopCoderX hesabınızdaki model kimliğini `provider/model-id` biçimini kullanır. Örneğin, [HopCoderX Bdr](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `HopCoderX/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `HopCoderX models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/TaimoorSiddiquiOfficial/hopcoderx).
:::
