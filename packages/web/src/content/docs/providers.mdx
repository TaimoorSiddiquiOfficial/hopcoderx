---
title: Providers
description: Using any LLM provider in HopCoderX.
---

import config from "../../../config.mjs"
export const console = config.console

HopCoderX uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your HopCoderX config.
...
You can customize the providers through the `provider` section in your HopCoderX
config.
...
HopCoderX Bdr is a list of models provided by the HopCoderX team that have been
tested and verified to work well with HopCoderX. [Learn more](/docs/bdr).

:::tip
If you are new, we recommend starting with HopCoderX Bdr.
:::

1. Run the `/connect` command in the TUI, select HopCoderX, and head to [hopcoderx.dev/auth](https://hopcoderx.dev/auth).
...
It works like any other provider in HopCoderX and is completely optional to use.
...
To use Amazon Bedrock with HopCoderX:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running HopCoderX:
   ...
   ***
   ...
   For project-specific or persistent configuration, use `HopCoderX.json`:
...
:::info
Using your Claude Pro/Max subscription in HopCoderX is not officially supported by [Anthropic](https://anthropic.com).
:::
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for HopCoderX to work properly.
   :::
...
5. Run the `/models` command to select a model.
   ...
   You can also add models through your HopCoderX config.
...
HopCoderX uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Bdr. To lock HopCoderX
to only use your own GitLab-hosted instance, add the following to your
`HopCoderX.json` file. It is also recommended to disable session sharing.
...
More documentation on [HopCoderX-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth) homepage.
...
To use your GitHub Copilot subscription with HopCoderX:
...
To use Google Vertex AI with HopCoderX:
...
In the event you see a feature or model from Helicone that isn't configured automatically through HopCoderX, you can always configure it yourself.
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [HopCoderX-helicone-session](https://github.com/H2Shami/HopCoderX-helicone-session) plugin to automatically log each HopCoderX conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each HopCoderX conversation listed as a separate session.
...
You can configure HopCoderX to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility
...
You can configure HopCoderX to use local models through LM Studio.
...
You can configure HopCoderX to use local models through Ollama.

:::tip
Ollama can automatically configure itself for HopCoderX. See the [Ollama integration docs](https://docs.ollama.com/integrations/HopCoderX) for details.
:::
...
To use Ollama Cloud with HopCoderX:
...
3. Copy the API key for use in HopCoderX.
...
6. **Important**: Before using cloud models in HopCoderX, you must pull the model information locally:
...
HopCoderX Bdr is a list of tested and verified models provided by the HopCoderX team. [Learn more](/docs/bdr).

1. Sign in to **<a href={console}>HopCoderX Bdr</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **HopCoderX Bdr**.

   ```txt
   /connect
   ```

3. Enter your HopCoderX API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your HopCoderX config.
...
5. You can also customize them through your HopCoderX config. Here's an example of specifying a provider
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with HopCoderX:
...
You can also customize models through your HopCoderX config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with HopCoderX. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
4. Create or update your `HopCoderX.json` file in your project directory:
...
The `limit` fields allow HopCoderX to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `HopCoderX auth list` to see if the credentials
...
2. For custom providers, check the HopCoderX config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your HopCoderX config.
