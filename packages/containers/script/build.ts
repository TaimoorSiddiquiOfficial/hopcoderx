...
const reg = process.env.REGISTRY ?? "ghcr.io/TaimoorSiddiquiOfficial"
...
const setup = async () => {
  ...
  if (list.includes("HopCoderX")) {
    await $`docker buildx use HopCoderX`
    return
  }
  await $`docker buildx create --name HopCoderX --use`
}
