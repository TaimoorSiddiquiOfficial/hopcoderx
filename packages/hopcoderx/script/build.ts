...
const refreshModelsSnapshot =
  process.argv.includes("--refresh-models-snapshot") || process.env.HOPCODERX_REFRESH_MODELS_SNAPSHOT === "1"
if (refreshModelsSnapshot || !fs.existsSync(snapshotPath)) {
  await import("./models-snapshot")
} else {
...
for (const item of targets) {
  ...
  try {
    buildResult = await Bun.build({
      ...
      define: {
        HOPCODERX_VERSION: `'${Script.version}'`,
        HOPCODERX_MIGRATIONS: JSON.stringify(migrations),
        OTUI_TREE_SITTER_WORKER_PATH: bunfsRoot + workerRelativePath,
        HOPCODERX_WORKER_PATH: workerPath,
        HOPCODERX_CHANNEL: `'${Script.channel}'`,
        HOPCODERX_LIBC: item.os === "linux" ? `'${item.abi ?? "glibc"}'` : "",
      },
    })
  } catch (err) {
