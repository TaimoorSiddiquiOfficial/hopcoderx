---
name: node-connect
description: Diagnose HopCoderX node connection and pairing failures for Android, iOS, and macOS companion apps. Use when QR/setup code/manual connect fails, local Wi-Fi works but VPS/tailnet does not, or errors mention pairing required, unauthorized, bootstrap token invalid or expired, gateway.bind, gateway.remote.url, Tailscale, or plugins.entries.device-pair.config.publicUrl.
---

# Node Connect

Goal: find the one real route from node -> gateway, verify HopCoderX is advertising that route, then fix pairing/auth.
...
- whether `HopCoderX devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `HopCoderX qr --json`. It uses the same setup-code payload Android scans.

```bash
HopCoderX config get gateway.mode
HopCoderX config get gateway.bind
HopCoderX config get gateway.tailscale.mode
HopCoderX config get gateway.remote.url
HopCoderX config get gateway.auth.mode
HopCoderX config get gateway.auth.allowTailscale
HopCoderX config get plugins.entries.device-pair.config.publicUrl
HopCoderX qr --json
HopCoderX devices list
HopCoderX nodes status
```

If this HopCoderX instance is pointed at a remote gateway, also run:

```bash
HopCoderX qr --remote --json
```
...
`HopCoderX qr --json` success means:
...
If `HopCoderX qr --json` says `Gateway is only bound to loopback`:
...
```bash
HopCoderX devices list
HopCoderX devices approve --latest
```
...
- `HopCoderX devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run HopCoderX qr again, rescan, then approve the pending device pairing.`
