...
1. Download the runtime for your OS (extracts into `$HopCoderX_STATE_DIR/tools/sherpa-onnx-tts/runtime`, default `~/.HopCoderX/tools/sherpa-onnx-tts/runtime`)
2. Download a voice model (extracts into `$HopCoderX_STATE_DIR/tools/sherpa-onnx-tts/models`, default `~/.HopCoderX/tools/sherpa-onnx-tts/models`)

Resolve the active state directory first:

```bash
STATE_DIR="${HopCoderX_STATE_DIR:-$HOME/.HopCoderX}"
```

Then write those resolved paths into the active HopCoderX config file (`$HopCoderX_CONFIG_PATH`, default `~/.HopCoderX/HopCoderX.json`):
