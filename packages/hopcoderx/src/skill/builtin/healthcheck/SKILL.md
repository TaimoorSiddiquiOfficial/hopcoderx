---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for HopCoderX deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, HopCoderX cron scheduling for periodic checks, or version status checks on a machine running HopCoderX (laptop, workstation, Pi, VPS).
---

# HopCoderX Host Hardening

## Overview

Assess and harden the host running HopCoderX, then align it to a user-defined risk tolerance without breaking access. Use HopCoderX security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim HopCoderX changes the host firewall, SSH, or OS updates; it does not.
...
5. HopCoderX gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if HopCoderX can access sensitive data.
...
### 2) Run HopCoderX security audits (read-only)

As part of the default read-only checks, run `HopCoderX security audit --deep`. Only offer alternatives if the user requests them:

1. `HopCoderX security audit` (faster, non-probing)
2. `HopCoderX security audit --json` (structured output)

Offer to apply HopCoderX safe defaults (numbered):

1. `HopCoderX security audit --fix`

Be explicit that `--fix` only tightens HopCoderX defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check HopCoderX version/update status (read-only)

As part of the default read-only checks, run `HopCoderX update status`.
...
- Credential hygiene notes (location of HopCoderX creds, prefer disk encryption)
...
- HopCoderX security audit (re-run)
...
After HopCoderX install or first hardening pass, run at least one baseline audit and version check:

- `HopCoderX security audit`
- `HopCoderX security audit --deep`
- `HopCoderX update status`

Ongoing monitoring is recommended. Use the HopCoderX cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `HopCoderX cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `HopCoderX update status`
...
Before creating, `HopCoderX cron list` and match on exact `name`. If found, `HopCoderX cron edit <id> ...`.
If not found, `HopCoderX cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `HopCoderX update status` (preferred for source checkouts and channels)
2. `npm view HopCoderX version` (published npm version)

## HopCoderX command accuracy

Use only supported commands and flags:

- `HopCoderX security audit [--deep] [--fix] [--json]`
- `HopCoderX status` / `HopCoderX status --deep`
- `HopCoderX health --json`
- `HopCoderX update status`
- `HopCoderX cron add|list|runs|run`

Do not invent CLI flags or imply HopCoderX enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by HopCoderX compaction:
