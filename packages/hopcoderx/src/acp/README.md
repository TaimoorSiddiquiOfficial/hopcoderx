...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for HopCoderX.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal HopCoderX sessions
...
HopCoderX acp --cwd /path/to/project
```
...
```bash
HOPCODERX_ENABLE_QUESTION_TOOL=1 HopCoderX acp
```
...
- **Terminal Integration**: Full terminal support via HopCoderX's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | HopCoderX acp
```
...
### Mapping to HopCoderX

ACP sessions map cleanly to HopCoderX's internal session model:
