...
export namespace Config {
  ...
  export const state = Instance.state(async () => {
    ...
    const directories = [
      ...
      ...(!Flag.HOPCODERX_DISABLE_PROJECT_CONFIG
        ...
        : []),
      // Always scan ~/.hopcoderx/ (user home directory)
      ...(await Array.fromAsync(
        Filesystem.up({
          targets: [".hopcoderx"],
  ...
  export async function installDependencies(dir: string) {
    ...
    json.dependencies = {
      ...json.dependencies,
      "@hopcoderx/plugin": targetVersion,
    }
  ...
  async function needsInstall(dir: string) {
    ...
    const depVersion = dependencies["@hopcoderx/plugin"]
    if (!depVersion) return true

    const targetVersion = Installation.isLocal() ? "latest" : Installation.VERSION
    if (targetVersion === "latest") {
      const isOutdated = await PackageRegistry.isOutdated("@hopcoderx/plugin", depVersion, dir)
      if (!isOutdated) return false
      log.info("Cached version is outdated, proceeding with install", {
        pkg: "@hopcoderx/plugin",
  ...
  async function loadCommand(dir: string) {
    ...
    for (const item of await Glob.scan("{command,commands}/**/*.md", {
      ...
      const patterns = ["/.hopcoderx/command/", "/.hopcoderx/commands/", "/command/", "/commands/"]
  ...
  async function loadAgent(dir: string) {
    ...
    for (const item of await Glob.scan("{agent,agents}/**/*.md", {
      ...
      const patterns = ["/.hopcoderx/agent/", "/.hopcoderx/agents/", "/agent/", "/agents/"]
      const file = rel(item, patterns) ?? path.basename(item)
  ...
  export const Keybinds = z
    .object({
      ...
      worktree_list: z.string().optional().default("<leader>w").describe("List and manage git worktrees"),
  ...
  export const Info = z
    .object({
      ...
      command: z
        ...
        .optional()
        .describe("Command configuration, see https://HopCoderX.ai/docs/commands"),
      ...
      agent: z
        ...
        .catchall(Agent)
        .optional()
        .describe("Agent configuration, see https://HopCoderX.ai/docs/agents"),
      provider: z
  ...
  export const global = lazy(async () => {
    let result: Info = pipe(
      ...
      mergeDeep(await loadFile(path.join(Global.Path.config, "hopcoderx.json"))),
      mergeDeep(await loadFile(path.join(Global.Path.config, "hopcoderx.jsonc"))),
    )
  ...
  function globalConfigFile() {
    const candidates = ["hopcoderx.jsonc", "hopcoderx.json", "config.json"].map((file) =>
