...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run HopCoderX",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.exitCode === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for HopCoderX session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found HopCoderX session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await $`hopcoderx import ${sessionUrl}`.nothrow()
        ...
        UI.println()
        UI.println("Starting HopCoderX...")
        UI.println()

        // Launch HopCoderX TUI with session ID if available
        const { spawn } = await import("child_process")
        const hopcoderxArgs = sessionId ? ["-s", sessionId] : []
        const hopcoderxProcess = spawn("hopcoderx", hopcoderxArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          hopcoderxProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`HopCoderX exited with code ${code}`))
          })
          hopcoderxProcess.on("error", reject)
        })
      },
    })
  },
