...
import { createHopCoderXClient, type Message, type HopCoderXClient, type ToolPart } from "@hopcoderx/sdk/v2"
...
export const RunCommand = cmd({
  ...
  handler: async (args) => {
    ...
    async function session(sdk: HopCoderXClient) {
    ...
    async function share(sdk: HopCoderXClient, sessionID: string) {
    ...
    async function execute(sdk: HopCoderXClient) {
    ...
    await bootstrap(process.cwd(), async () => {
      ...
      const sdk = createHopCoderXClient({ baseUrl: "http://opencode.internal", fetch: fetchFn })
      await execute(sdk)
    })
  },
})
