...
export namespace Provider {
  ...
  const CUSTOM_LOADERS: Record<string, CustomLoader> = {
    ...
    "hopcoderx-bdr": async (input) => {
      ...
      if (virtualKey) {
        // Sync available models from the gateway — admin panel is the source of truth.
        // Fetch /v1/models and patch database["hopcoderx-bdr"].models so the CLI
        // model picker always reflects exactly what the admin has enabled.
        try {
          const bdrBase = "https://hopcoderx-bdr.taimoorrehman-sid.workers.dev/v1"
          const res = await fetch(`${bdrBase}/models`, {
            ...
            if (json.data?.length) {
              input.models = Object.fromEntries(
                json.data.map((m: any) => {
                  ...
                  const prov = (m.owned_by as string) || id.split("/")[0]
                  const ctx = (m.context_length as number) || 128000
                  const vision = ["openai", "google", "anthropic"].includes(prov)
                  const tools = prov !== "workers-ai"
                  const reasoning = id.includes("o1") || id.includes("o3") || id.includes("think")
                  const model: Model = {
                    id,
                    ...
                    options: {},
                    cost: {
                      input: ((m.pricing?.prompt as number) ?? 0),
                      output: ((m.pricing?.completion as number) ?? 0),
                      cache: { read: 0, write: 0 },
                    },
                    limit: { context: ctx, output: Math.min(Math.floor(ctx / 8), 16384) },
                    capabilities: {
                      temperature: !reasoning,
                      reasoning,
                      attachment: vision,
                      toolcall: tools,
                      input: { text: true, audio: false, image: vision, video: false, pdf: vision },
          ...
          // gateway unreachable — model list stays empty; error will surface when a model is invoked
        }
        return {
          autoload: true,
          options: {
  ...
  const state = Instance.state(async () => {
    ...
    // Registered directly in code so every user sees it in `auth login` and the
    // CUSTOM_LOADER can always resolve it. Model list starts empty — the
    // CUSTOM_LOADER fetches live models from the admin-managed /v1/models endpoint
    // so the web panel is always the single source of truth.
