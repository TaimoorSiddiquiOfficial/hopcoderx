...
export namespace ProviderTransform {
  export const OUTPUT_TOKEN_MAX = Flag.HOPCODERX_EXPERIMENTAL_OUTPUT_TOKEN_MAX || 32_000
    ...
    if (model.api.id.includes("claude")) {
      return msgs.map((msg) => {
        if ((msg.role === "assistant" || msg.role === "tool") && Array.isArray(msg.content)) {
          ;(msg as any).content = (msg.content as any[]).map((part: any) => {
  ...
  export function options(input: {
    ...
    if (
      ...
      (input.model.providerID === "hopcoderx" && ["kimi-k2-thinking", "glm-4.6"].includes(input.model.api.id))
    ) {
    ...
    if (input.model.api.id.includes("gpt-5") && !input.model.api.id.includes("gpt-5-chat")) {
      ...
      if (input.model.providerID.startsWith("hopcoderx")) {
        result["promptCacheKey"] = input.sessionID
