...
const sandbox = await fs.mkdtemp(path.join(os.tmpdir(), "HopCoderX-e2e-"))
const keepSandbox = process.env.HOPCODERX_E2E_KEEP_SANDBOX === "1"

const serverEnv = {
  ...process.env,
  HOPCODERX_DISABLE_SHARE: process.env.HOPCODERX_DISABLE_SHARE ?? "true",
  HOPCODERX_DISABLE_LSP_DOWNLOAD: "true",
  HOPCODERX_DISABLE_DEFAULT_PLUGINS: "true",
  HOPCODERX_EXPERIMENTAL_DISABLE_FILEWATCHER: "true",
  HOPCODERX_TEST_HOME: path.join(sandbox, "home"),
  XDG_DATA_HOME: path.join(sandbox, "share"),
  XDG_CACHE_HOME: path.join(sandbox, "cache"),
  XDG_CONFIG_HOME: path.join(sandbox, "config"),
  XDG_STATE_HOME: path.join(sandbox, "state"),
  HOPCODERX_E2E_PROJECT_DIR: repoDir,
  HOPCODERX_E2E_SESSION_TITLE: "E2E Session",
  HOPCODERX_E2E_MESSAGE: "Seeded for UI e2e",
  HOPCODERX_E2E_MODEL: "HopCoderX/gpt-5-nano",
  HOPCODERX_CLIENT: "app",
...
const runnerEnv = {
  ...
  VITE_HOPCODERX_SERVER_HOST: "127.0.0.1",
  VITE_HOPCODERX_SERVER_PORT: String(serverPort),
...
try {
  ...
  if (seedExit !== 0) {
    ...
    const log = await import("../../hopcoderx/src/util/log")
    const install = await import("../../hopcoderx/src/installation")
    await log.Log.init({
      print: true,
    ...
    const servermod = await import("../../hopcoderx/src/server/server")
    inst = await import("../../hopcoderx/src/project/instance")
    server = servermod.Server.listen({ port: serverPort, hostname: "127.0.0.1" })
    console.log(`HopCoderX server listening on http://127.0.0.1:${serverPort}`)
