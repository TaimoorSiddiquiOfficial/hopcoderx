...
export const popularProviders = ["hopcoderx", "anthropic", "github-copilot", "openai", "google", "openrouter", "vercel"]
...
export function useProviders() {
  ...
  const paid = createMemo(() =>
    connected().filter((p) => p.id !== "hopcoderx" || Object.values(p.models).find((m) => m.cost?.input)),
  )
