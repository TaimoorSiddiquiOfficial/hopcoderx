    ...
    await globalSync
      ...
      .then(() => {
        showToast({
          variant: "success",
          icon: "circle-check",
          title: `Command "${commandName}" Deleted`,
          description: `The command "${commandName}" has been removed from your configuration.`,
        })
      })
      .catch((err: unknown) => {
    ...
    <div class="flex flex-col h-full overflow-y-auto no-scrollbar px-4 pb-10 sm:px-10 sm:pb-10">
      <div class="sticky top-0 z-10 bg-[linear-gradient(to_bottom,var(--surface-stronger-non-alpha)_calc(100%_-_24px),transparent)]">
        <div class="flex flex-col gap-4 pt-6 pb-6 max-w-[720px]">
          ...
          <p class="text-14-regular text-text-weak">Manage your custom AI commands</p>
        </div>
      </div>

      <div class="flex flex-col gap-8 max-w-[720px]">
        <div class="flex flex-col gap-1">
          <h3 class="text-14-medium text-text-strong pb-2">Configured Commands</h3>
                    ...
                    <div class="flex flex-col min-w-0 flex-1">
                      <div class="flex items-center gap-3">
                        ...
                        <Show when={command.agent}>
                          <span class="text-12-regular text-text-weak px-2 py-0.5 rounded bg-surface-stronger-base">{command.agent}</span>
                        </Show>
                      </div>
        ...
        <div class="flex flex-col gap-2">
          <p class="text-14-regular text-text-weak">
            Edit commands in your hopcoderx.json config file
          </p>
        </div>
      </div>
    </div>
