...
import os from "os"
import { BdrData } from "../src/model"

const root = path.resolve(process.cwd(), "..", "..", "..")
const models = await $`bun sst secret list`.cwd(root).text()
const PARTS = 30

// read the line starting with "BDR_MODELS"
const lines = models.split("\n")
const oldValues = Array.from({ length: PARTS }, (_, i) => {
  const value = lines
    .find((line) => line.startsWith(`BDR_MODELS${i + 1}=`))
    ?.split("=")
    .slice(1)
    .join("=")
  if (!value) throw new Error(`BDR_MODELS${i + 1} not found`)
  return value
})
...
BdrData.validate(JSON.parse(newValue))
...
await envFile.write(newValues.map((v, i) => `BDR_MODELS${i + 1}="${v.replace(/"/g, '\\"')}"`).join("\n"))
